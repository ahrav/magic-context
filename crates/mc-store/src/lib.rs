...
impl McStore {
    ...
    /// The loader returns an empty core and default metadata when no session row exists; the classifier then bootstraps.
    ...
    fn apply_state_sync(
        ...
        let default_core_json = serde_json::to_string(&CoreState::empty())
    ...
    /// The reset writes an empty `CoreState` and default `ModuleMeta` with a bumped revert epoch.
