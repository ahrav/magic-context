...
const COMPONENTS: &[(&str, &str)] = &[
    ...
    (
        "admission_decisions",
        r#"CREATE TABLE admission_decisions(admission_decision_id TEXT PRIMARY KEY,candidate_id TEXT REFERENCES candidates(candidate_id) ON DELETE SET NULL,candidate_ref TEXT,candidate_kind TEXT,candidate_payload_digest TEXT CHECK(candidate_payload_digest IS NULL OR length(candidate_payload_digest)=64),subject_object_id TEXT REFERENCES object_registry(object_id),source_kind TEXT NOT NULL,source_id TEXT NOT NULL,source_revision INTEGER NOT NULL,source_class TEXT NOT NULL,taint_class TEXT NOT NULL,event_kind TEXT NOT NULL,maturity TEXT NOT NULL,effective_maturity TEXT NOT NULL,disposition TEXT NOT NULL,visibility TEXT NOT NULL,outcome TEXT NOT NULL,sensitivity_class TEXT NOT NULL,policy_revision INTEGER NOT NULL,reason TEXT NOT NULL,evidence_id TEXT REFERENCES evidence_meta(evidence_id) ON DELETE RESTRICT,approval_object_id TEXT REFERENCES object_registry(object_id) ON DELETE RESTRICT,elevated_support INTEGER NOT NULL DEFAULT 0 CHECK(elevated_support IN (0,1)),commit_seq INTEGER REFERENCES commit_log(commit_seq) ON DELETE RESTRICT,decided_at INTEGER NOT NULL) STRICT; CREATE INDEX idx_admission_candidate_latest ON admission_decisions(candidate_id,commit_seq DESC,admission_decision_id DESC); CREATE INDEX idx_admission_candidate_ref ON admission_decisions(candidate_ref,subject_object_id,commit_seq) WHERE candidate_ref IS NOT NULL; CREATE INDEX idx_admission_subject_latest ON admission_decisions(subject_object_id,commit_seq DESC,admission_decision_id DESC); CREATE INDEX idx_admission_evidence_fk ON admission_decisions(evidence_id); CREATE INDEX idx_admission_approval_fk ON admission_decisions(approval_object_id); CREATE INDEX idx_admission_commit_fk ON admission_decisions(commit_seq,admission_decision_id); CREATE INDEX idx_admission_source ON admission_decisions(source_kind,source_id,source_revision,decided_at); CREATE INDEX idx_admission_source_latest ON admission_decisions(source_kind,source_id,source_revision,commit_seq,admission_decision_id) WHERE subject_object_id IS NULL;"#,
    ),
    (
        "decisions",
