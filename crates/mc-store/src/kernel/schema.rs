...
pub const KERNEL_SCHEMA_COMPONENT_NAMES: [&str; 35] = [
    "commit_log",
    "change_event",
    "outbox",
    "operation_receipts",
...
const COMPONENTS: [(&str, &str); KERNEL_SCHEMA_COMPONENT_NAMES.len()] = [
    (
        "commit_log",
        r#"CREATE TABLE commit_log(commit_seq INTEGER PRIMARY KEY AUTOINCREMENT,transaction_id TEXT NOT NULL UNIQUE CHECK(instr(transaction_id,'REDACTED')=0),writer_epoch INTEGER NOT NULL,producer TEXT NOT NULL DEFAULT 'legacy' CHECK(instr(producer,'REDACTED')=0),operation_key TEXT NOT NULL DEFAULT 'legacy' CHECK(instr(operation_key,'REDACTED')=0),request_digest TEXT NOT NULL DEFAULT '',recorded_at INTEGER NOT NULL,actor TEXT NOT NULL CHECK(instr(actor,'REDACTED')=0),cause TEXT NOT NULL) STRICT; CREATE UNIQUE INDEX idx_commit_operation ON commit_log(producer,operation_key);"#,
    ),
    (
        "change_event",
    ...
    (
        "operation_receipts",
        r#"CREATE TABLE operation_receipts(receipt_id TEXT PRIMARY KEY CHECK(instr(receipt_id,'REDACTED')=0),producer TEXT NOT NULL CHECK(instr(producer,'REDACTED')=0),operation_key TEXT NOT NULL CHECK(instr(operation_key,'REDACTED')=0),request_digest TEXT NOT NULL,commit_seq INTEGER REFERENCES commit_log(commit_seq) ON DELETE RESTRICT,result_payload TEXT NOT NULL,created_at INTEGER NOT NULL,UNIQUE(producer,operation_key)) STRICT; CREATE INDEX idx_receipts_commit_fk ON operation_receipts(commit_seq);"#,
    ),
    (
        "scan_batches",
        r#"CREATE TABLE scan_batches(scan_batch_id TEXT PRIMARY KEY CHECK(length(scan_batch_id)=32),commit_seq INTEGER UNIQUE REFERENCES commit_log(commit_seq) ON DELETE CASCADE,created_at INTEGER NOT NULL) STRICT; CREATE INDEX idx_scan_batches_commit_fk ON scan_batches(commit_seq);"#,
    ),
    (
        "field_scans",
        r#"CREATE TABLE field_scans(scan_id TEXT PRIMARY KEY CHECK(length(scan_id)=32),scan_batch_id TEXT NOT NULL REFERENCES scan_batches(scan_batch_id) ON DELETE CASCADE,detector_id TEXT NOT NULL,detector_revision TEXT NOT NULL,semantic_digest TEXT CHECK(semantic_digest IS NULL OR length(semantic_digest)=64),finding_count INTEGER NOT NULL CHECK(finding_count>=0)) STRICT; CREATE INDEX idx_field_scans_batch_fk ON field_scans(scan_batch_id,scan_id);"#,
    ),
    (
        "scan_owner_copies",
        r#"CREATE TABLE scan_owner_copies(owner_copy_id TEXT PRIMARY KEY CHECK(length(owner_copy_id)=32),scan_id TEXT NOT NULL REFERENCES field_scans(scan_id) ON DELETE CASCADE,owner_kind TEXT NOT NULL CHECK(owner_kind IN ('commit_log','change_event','object_registry','outbox','operation_receipt','extraction_run','staging_candidate','alignment_projection')),field_id TEXT NOT NULL CHECK(field_id IN ('producer','operation_key','actor','cause','domain_id','object_id','name','source_kind','source_id','replaced_object_id','retired_object_id','result_payload','extraction_run_id','extractor','repository_id','revision','candidate_id','candidate_kind','payload','decision_id','observation_id','alignment_kind','alignment_payload','consumer_id','operator_id','reason')),owner_commit_seq INTEGER REFERENCES commit_log(commit_seq) ON DELETE CASCADE) STRICT; CREATE INDEX idx_scan_owner_scan_fk ON scan_owner_copies(scan_id,owner_copy_id); CREATE INDEX idx_scan_owner_lifetime ON scan_owner_copies(owner_kind,owner_commit_seq,owner_copy_id);"#,
    ),
    (
        "scan_detections",
        r#"CREATE TABLE scan_detections(scan_id TEXT NOT NULL REFERENCES field_scans(scan_id) ON DELETE CASCADE,detection_ordinal INTEGER NOT NULL CHECK(detection_ordinal>=0),rule_id TEXT NOT NULL,detector_revision TEXT NOT NULL,exactness TEXT NOT NULL CHECK(exactness='exact'),label_id TEXT NOT NULL CHECK(label_id IN ('anthropic_api_key','openai_api_key','github_pat','github_token','huggingface_token','aws_access_key_id','slack_token','google_api_key','bearer','jwt','api_key','auth_token','password','key','token','secret')),span_kind TEXT NOT NULL CHECK(span_kind='value'),action TEXT NOT NULL CHECK(action IN ('substitute','reject')),PRIMARY KEY(scan_id,detection_ordinal)) STRICT;"#,
    ),
    (
        "writer_fence",
    ...
    (
        "extraction_runs",
        r#"CREATE TABLE extraction_runs(extraction_run_id TEXT PRIMARY KEY,extractor TEXT NOT NULL,source_kind TEXT,source_id TEXT,source_revision INTEGER,sensitivity_class TEXT NOT NULL,provenance_witness BLOB NOT NULL,redaction_metadata BLOB NOT NULL,scan_batch_id TEXT NOT NULL REFERENCES scan_batches(scan_batch_id) ON DELETE RESTRICT,started_at INTEGER NOT NULL,heartbeat_at INTEGER NOT NULL,lease_expires_at INTEGER NOT NULL,terminal_state TEXT,terminal_at INTEGER) STRICT; CREATE INDEX idx_runs_ttl ON extraction_runs(terminal_at,lease_expires_at,extraction_run_id); CREATE INDEX idx_runs_heartbeat ON extraction_runs(terminal_at,heartbeat_at,extraction_run_id); CREATE INDEX idx_runs_scan_batch_fk ON extraction_runs(scan_batch_id);"#,
    ),
    (
        "candidates",
        r#"CREATE TABLE candidates(candidate_id TEXT PRIMARY KEY,extraction_run_id TEXT NOT NULL REFERENCES extraction_runs(extraction_run_id) ON DELETE CASCADE,candidate_kind TEXT NOT NULL,payload TEXT NOT NULL,sensitivity_class TEXT NOT NULL,provenance_witness BLOB NOT NULL,redaction_metadata BLOB NOT NULL,scan_batch_id TEXT NOT NULL REFERENCES scan_batches(scan_batch_id) ON DELETE RESTRICT,created_at INTEGER NOT NULL,heartbeat_at INTEGER NOT NULL,lease_expires_at INTEGER NOT NULL,terminal_state TEXT,terminal_at INTEGER) STRICT; CREATE INDEX idx_candidates_run_fk ON candidates(extraction_run_id,candidate_id); CREATE INDEX idx_candidates_ttl ON candidates(terminal_at,lease_expires_at,candidate_id); CREATE INDEX idx_candidates_scan_batch_fk ON candidates(scan_batch_id);"#,
    ),
    (
        "candidate_scores",
    ...
    (
        "alignment_projection",
        r#"CREATE TABLE alignment_projection(decision_id TEXT NOT NULL REFERENCES decisions(decision_id) ON DELETE CASCADE,observation_id TEXT NOT NULL REFERENCES observations(observation_id) ON DELETE CASCADE,alignment_kind TEXT NOT NULL,alignment_payload TEXT,scan_batch_id TEXT NOT NULL REFERENCES scan_batches(scan_batch_id) ON DELETE RESTRICT,built_through_commit_seq INTEGER NOT NULL REFERENCES commit_log(commit_seq),PRIMARY KEY(decision_id,observation_id)) STRICT; CREATE INDEX idx_alignment_observation_fk ON alignment_projection(observation_id,decision_id); CREATE INDEX idx_alignment_built ON alignment_projection(built_through_commit_seq,decision_id); CREATE INDEX idx_alignment_scan_batch_fk ON alignment_projection(scan_batch_id);"#,
    ),
    (
        "mc_kernel_format_marker",
