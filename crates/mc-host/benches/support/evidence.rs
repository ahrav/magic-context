...
pub struct ArmId {
    ...
    /// `pair` orders logical CPUs as (initiator/load, responder/host).
...
pub struct Manifest {
    ...
    /// Affinity data records the requested pair and singleton affinity readbacks after pinning.
    pub affinity: Option<serde_json::Value>,
    pub outcomes: Option<OutcomeCounts>,
    /// Histograms record only post-warmup values.
    pub recorded_samples: Option<u64>,
    /// Out-of-range records count as rejected terminal outcomes.
    pub histogram_rejected: Option<u64>,
    pub skip_reason: Option<String>,
    pub fail_reason: Option<String>,
    pub sidecars: Vec<Sidecar>,
    /// `collection` must remain uniform within an arm because merging different warmup, count, rate, cap, or depth settings combines non-comparable experiments.
    /// Absent `collection` defaults to `None`.
    #[serde(default)]
    pub collection: Option<serde_json::Value>,
    /// Scalar summaries are written only after all sidecars exist.
...
/// The rename publishes the terminal manifest atomically.
/// `rename` prevents readers from observing a torn final manifest.
...
/// Rewrites each leftover running manifest under `run_dir` as an `interrupted` manifest, retaining the attempt for diagnosis.
...
pub fn load_attempts(run_dir: &Path) -> Result<Vec<LoadedAttempt>, String> {
    ...
    let entries =
        std::fs::read_dir(run_dir).map_err(|err| format!("{}: {err}", run_dir.display()))?;
    // Enumeration errors fail the load because omitting a finalized attempt would publish a partial aggregate.
    ...
    for dir in dirs {
        ...
        let manifest: Manifest = serde_json::from_slice(&bytes)
            .map_err(|err| format!("{}: {err}", manifest_path.display()))?;
        // Every manifest must match `SCHEMA_VERSION`; pairwise `compatible` checks cannot detect a directory of uniformly unsupported schemas.
...
/// `collection` mismatches can change rates, depths, or operation counts.
/// Collection mismatches must not pool workloads with different rates, depths, or operation counts.
...
pub fn merge_arm_histograms(
    ...
    for attempt in &complete {
        ...
        // `merge_arm_histograms` rejects sidecars whose histogram configuration differs from the manifest because the manifest determines the merged histogram's percentile precision.
...
pub fn paired_gaps(attempts: &[LoadedAttempt]) -> Result<Vec<GapRow>, String> {
    ...
    for attempt in &complete {
        // Pairing checks only the shared environment because arms may use different collection configurations.
        // collection schema.
        ...
        match m.arm.name.as_str() {
            ARM_ATOMIC => {
                ...
                // The gap scalar must agree with the checksummed sidecar from which it derives.
                // Sidecar checksums do not protect the manifest's `results` fields.
                // An edited or corrupted scalar could otherwise steer gap statistics while every sidecar checksum verifies.
                verify_sidecars(attempt)?;
...
/// Odd-indexed blocks reverse `arms` to alternate each arm's position across adjacent block pairs.
/// Each adjacent forward/reversed block pair gives every arm complementary positions.
