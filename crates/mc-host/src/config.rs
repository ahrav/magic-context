...
impl HostLimits {
    pub fn validate(&self) -> Result<(), ConfigError> {
        ...
        // Tokio semaphores hold at most usize::MAX >> 3 permits; byte-granular
        // charges must fit, and single acquisitions use u32 counts.
        if self.max_resident_bytes > u32::MAX as u64 {
...
impl HostConfig {
    pub fn validate(&self) -> Result<(), ConfigError> {
        ...
        let auth_message_bytes = serde_json::to_vec(&subc_transport::ServerProof {
            daemon_id: [0; subc_transport::DAEMON_ID_LEN],
            server_nonce: [0; subc_transport::NONCE_LEN],
            daemon_ver: self.daemon_ver.clone(),
            server_proof: [0; subc_transport::PROOF_LEN],
        })
        ...
        let connection_file_bytes = serde_json::to_vec_pretty(&subc_transport::ConnectionInfo {
            ...
            key: vec![0; subc_transport::KEY_LEN],
            daemon_id: [0; subc_transport::DAEMON_ID_LEN],
            pid: u32::MAX,
            daemon_ver: self.daemon_ver.clone(),
        })
