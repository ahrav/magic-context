...
| Safelist-divergence-1 | `CONTEXT_SAFELIST` in `crates/mc-secret-scanner/src/rules.rs` | Accepted as deliberate divergence | The context safelist omits the two upstream patterns that match documentation wording alone, `for example|sample config|example config` and `__tests__|fixtures|mocks`. Every remaining pattern describes the candidate's own syntax or immediate delimiters. Upstream applies the safelist over a 256-byte window, so a prose-only pattern suppressed a live credential quoted beside documentation; the value safelist, not those two patterns, is what keeps placeholder values quiet | `documentation_wording_near_a_credential_does_not_excuse_it` and `a_placeholder_value_stays_unredacted_next_to_its_own_label` in `crates/mc-core/tests/redaction_policy.rs`, and `no_context_pattern_matches_prose_without_a_credential_shaped_neighbour` in `crates/mc-secret-scanner/src/rules.rs` |
