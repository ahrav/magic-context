...
const schedulerMod = await import(
    resolve("./src/features/magic-context/scheduler")
);
const eventResolvers = await import(
    resolve("./src/hooks/magic-context/event-resolvers")
);
const overflowMod = await import(
    resolve("./src/features/magic-context/overflow-detection")
);
const escalation = await import(resolve("./src/shared/escalation-bands"));
const compartmentTrigger = await import(
    resolve("./src/hooks/magic-context/compartment-trigger")
);
const storageMeta = await import(
    resolve("./src/features/magic-context/storage-meta-persisted")
);
...
const thresholdCases = [
    {
        label: "percentage numeric below cap",
    ...
    { label: "percentage cap at 90", percentage_config: 95, fallback: 65 },
    {
        ...
        percentage_config: {
            default: 65,
            "anthropic/claude-sonnet-4-20250514": 72,
        },
    ...
    ...c,
    expected: resolveExecuteThreshold(
        c.percentage_config,
        c.model_key,
        c.fallback,
        {
...
const shouldExecuteSpecs: Array<{
    ...
    },
    {
        ...
        config: {
            ...
            executeThresholdTokens: { default: 32_000 },
        },
        sessionMeta: { lastResponseTime: 1_000, cacheTtl: "5m" },
    ...
    },
    {
        ...
        config: {
            executeThresholdPercentage: 65,
            executeThresholdTokens: { default: 50_000 },
        },
        sessionMeta: { lastResponseTime: 1_000, cacheTtl: "5m" },
    ...
    },
    {
        ...
        config: {
            executeThresholdPercentage: { default: 65, "openai/gpt-5.4": 58 },
        },
        sessionMeta: { lastResponseTime: 1_000, cacheTtl: "5m" },
...
const should_execute_cases = shouldExecuteSpecs.map((c) => ({
    label: c.label,
    config: schedulerConfig(
        c.config.executeThresholdPercentage,
        c.config.executeThresholdTokens,
    ),
    ...
    expected_threshold: resolveExecuteThreshold(
        c.config.executeThresholdPercentage,
        c.modelKey,
        65,
        {
...
const ttl_predicate_cases = [
    {
        label: "elapsed equals ttl",
        now_ms: 10_000 + ttl,
        last_response_time_ms: 10_000,
        ttl_ms: ttl,
    },
    {
        label: "elapsed ttl plus one",
        now_ms: 10_000 + ttl + 1,
        last_response_time_ms: 10_000,
        ttl_ms: ttl,
    },
    {
        label: "last response zero never hard expires",
        now_ms: ttl + 1,
    ...
    expected_hard_expired:
        c.last_response_time_ms > 0 &&
        c.now_ms - c.last_response_time_ms > c.ttl_ms,
}));
...
const overflowInputs: Array<[string, unknown]> = [
    ...
    [
        "nested-provider-error",
        {
            error: {
                message:
                    "Input token count 200000 exceeds the maximum of 128000",
            },
        },
    ],
...
const limitMessages = [
    ...
    [
        "above plausible ceiling rejected",
        "maximum context length is 10000001 tokens",
    ],
...
const parse_ttl_cases = ["5m", "30s", "2h", "1500", " 5m ", "garbage"].map(
    (ttl) => {
...
const constants = {
    default_execute_threshold_percentage:
        schema.DEFAULT_EXECUTE_THRESHOLD_PERCENTAGE,
    max_execute_threshold_percentage: resolveExecuteThreshold(
        999,
        undefined,
        65,
    ),
    ...
    emergency_drain_fallback_exit_percentage:
        storageMeta.EMERGENCY_DRAIN_FALLBACK_EXIT_PERCENTAGE,
    emergency_drain_failure_backoff_ms:
        storageMeta.EMERGENCY_DRAIN_FAILURE_BACKOFF_MS,
    emergency_drain_max_latch_ms: storageMeta.EMERGENCY_DRAIN_MAX_LATCH_MS,
    min_plausible_context_limit: parseReportedLimit(
        "maximum context length is 1024 tokens",
    )?.value,
    max_plausible_context_limit: parseReportedLimit(
        "maximum context length is 10000000 tokens",
    )?.value,
...
const outPath = join(
    ...
    "..",
    "testdata",
    "scheduler-golden.json",
);
writeFileSync(outPath, `${JSON.stringify(golden, null, 2)}\n`);
// eslint-disable-next-line no-console
console.log(
    `wrote scheduler golden (${should_execute_cases.length} shouldExecute, ${overflow_cases.length} overflow) → ${outPath}`,
);
