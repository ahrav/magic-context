...
//!
//! Three copies of the transform exist: `caveman.rs`, `reference` below, and the frozen copy
//! in `caveman_differential.rs`. The golden fixture is generated from `caveman.ts`.
...
mod reference {
    ...
    fn apply_ultra_connectives(text: &str) -> String {
        ULTRA_CONNECTIVE_REPLACEMENTS.iter().fold(
            text.to_string(),
            |working, (phrase, replacement)| {
...
const LEVELS: [(CavemanLevel, reference::CavemanLevel, &str); 3] = [
    (CavemanLevel::Lite, reference::CavemanLevel::Lite, "lite"),
    (CavemanLevel::Full, reference::CavemanLevel::Full, "full"),
    (CavemanLevel::Ultra, reference::CavemanLevel::Ultra, "ultra"),
];

#[test]
fn reference_matches_golden_fixture() {
    ...
    for case in &cases {
        ...
        assert_eq!(
            reference::compress(&case.text, reference::CavemanLevel::Full),
            case.full,
            "full: {:?}",
            case.text
        );
        assert_eq!(
            reference::compress(&case.text, reference::CavemanLevel::Ultra),
            case.ultra,
            "ultra: {:?}",
            case.text
        );
...
/// splitmix64; the corpus must be deterministic so failures replay.
struct Rng(u64);
...
fn fuzz_tokens() -> Vec<&'static str> {
    vec![
        // Pattern-set words, case variants, and boundary traps.
        "just",
        "really",
        "basically",
        "quite",
        ...
        "u: not user",
        "\nU: line\n",
        // Whitespace, including non-ASCII forms.
        " ",
        "  ",
        "\t",
        "\t ",
...
#[test]
fn production_matches_reference_on_fuzz_corpus() {
    let tokens = fuzz_tokens();
    let mut rng = Rng(0x5EED_CAFE);
    for case_idx in 0..400usize {
        let token_count = 1 + (rng.next() as usize) % 120;
        let mut text = String::new();
        for _ in 0..token_count {
            text.push_str(rng.pick(&tokens));
            // Sometimes join without a separator to exercise intra-word
            // overlaps and boundary rejections.
            if rng.next().is_multiple_of(2) {
                text.push_str(rng.pick(&[" ", "", " ", "\n", "  ", "\t"]));
            }
        }
        for (level, ref_level, name) in LEVELS {
            assert_eq!(
                compress(&text, level),
                reference::compress(&text, ref_level),
                "case {case_idx} level {name}: {text:?}"
            );
        }
    }
}
...
#[test]
fn production_matches_reference_on_tiled_golden() {
    let cases: Vec<GoldenCase> =
        serde_json::from_str(include_str!("../testdata/caveman-golden.json"))
            .expect("valid caveman golden");
    let blob: String = cases
        ...
        .collect::<Vec<_>>()
        .join("\n\n");
    let mut tiled = String::with_capacity(32 << 10);
    while tiled.len() < 32 << 10 {
        tiled.push_str(&blob);
        tiled.push_str("\n\n");
    }
    for (level, ref_level, name) in LEVELS {
        assert_eq!(
            compress(&tiled, level),
            reference::compress(&tiled, ref_level),
            "tiled golden, level {name}"
        );
    }
}
