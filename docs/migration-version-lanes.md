...
- **Binary order:** ship the OpenCode plugin, Pi plugin, CLI, and `ck-mc` from the same source revision **before** any process opens a database at v84. The first v84-capable process migrates on open; the schema fence (`LATEST_SUPPORTED_VERSION = 83`) then refuses older binaries.
