...
process and no new dependency. Eleven of the 24 records need nothing beyond
seeded state and one or two passes.
...
| **F7** frozen-unit seeding | A `CoreState` carrying a `strip:`, `red:`, `cav:` or reduced/sentinel frozen unit whose target block is in the measured tail or the rendered array | **Yes, and it needs no fault. This is the workhorse of the part.** `new_frozen_strip_units` (`transform.rs:10181-10339`) produces five strip classes and `apply_surface_strips` (`:10371-10458`) can collapse a whole message to one sentinel block (`:10388-10391`). Seeding one is ordinary fixture construction. The gap it exposes is that the cross-language corpus cannot: all 12 `nudge-hygiene-golden.json` cases pass `&CoreState::default()` and an empty `&HashSet` (`tail_hygiene.rs:1049-1056`), so the metric's `red:` arm (`:508`), caveman arm (`:526`) and absent strip arm are unreachable from the fixture that exists to stop cross-language drift |
...
All 24 records: twelve from lens A (rendered output, tags, determinism) and
twelve from lens B (the nudge overlay). "Non-vacuous today" means a developer can
...
| render-a-overlay-targets-stale-indices-after-full-drop-filter | One message containing a block the full-drop filter removes (`full_drop_tool_ids`, `:10839-10891`, which needs a frozen `red:` unit of kind `drop`) followed by **at least two** overlay-eligible blocks (F7) | **Partial, and it is one of two records blocked on an external question.** The index shift is verified from source: `filter_map` rebuilds `content` at `:12014-12021` and the overlay at `:12024-12031` passes the same unmodified `blocks` slice. What is not established is whether a real harness emits that message shape, which depends on the 4f codecs. A whole-message strip collapsing `content` to one block (`:10388`) creates the same shift with a smaller footprint and is constructible, so the weaker form is testable today; the misattribution form is not. Note also that the `block_index >= content.len()` guards at `:8227` and `:10400` convert the out-of-range half into a silently skipped overlay, which is a separate failure mode with the same cause |
...
| render-a-mint-batch-block-ids-are-unique-per-pass | `tag_mint_enabled`, plus either a duplicate projection block id or a stale tag baseline (F6) | **Partial, and it is the second externally-blocked record.** The duplicate-projection half is provably impossible: `apply_once` returns `TransformError::DuplicateBlockId` at `:3354-3356`, before the mint at `:3806`. The stale-baseline half needs `load_cached_tags` to serve rows whose block-id set differs from the store's, and both cached paths are fenced on the trigger-backed `generation` (`:7529`, `:7540`). Whether the `mc_tags` SQLite triggers advance `generation` for *every* mutation, not only inserts, is an `mc-store` question outside 4e. `poisoned_tag_baseline_refills_after_direct_sql_update` (`:23433`) is the nearest existing construction and the natural place to answer it |
...
| Property | Required faults and enabling state | Non-vacuous today |
| --- | --- | --- |
| nudge-b-frozen-todo-pair-retires-only-on-a-bust | `req.todo_tool_present == Some(true)`, `meta.last_todo_state` populated, a frozen pair present, and a pass whose plan is not `Hard`, `MigrateHard` or `Soft` (`transform.rs:4435-4439`); then a second pass with a **different** visible todowrite state on the same plan (F3) | **Yes.** Two passes and seeded metadata. The mechanism is verified: the defer short-circuit at `injection.rs:306-312`, the `Clear` arm at `:325-331` and its only caller `transform.rs:7461`, the stale-anchor drop at `transform.rs:7495-7500`, and `capture_todo_state_on_bust` refusing on a non-bust pass (`injection.rs:212-214`) so the metadata cannot move either. The 4b-side checks `defer_never_clears_but_bust_does` (`injection.rs:600-613`) and `defer_after_capture_replays_frozen_bytes` (`:739-771`) already assert this for `advance_injection`; what is uncovered is the `transform.rs` wrapper |
...
| nudge-b-injected-todo-pair-carries-no-provider-visible-provenance | A frozen pair and `synthetic_todo_enabled` (`transform.rs:5389` passes `tail_reclaim_enabled && !req.is_subagent`). No fault | **Yes.** One pass with a frozen pair. Three layers were verified, and the finding is that they disagree: CK wire carries `HarnessMeta::synthetic` (`mc-store/src/lib.rs:64-65`) so the host can always tell; the OpenCode native encoder emits `"syntheticTodoMarker": true` (`codec/opencode.rs:946`, reached from `:388`); the pi encoder emits nothing (`codec/pi.rs:582-607`) and has no production caller. In the provider array the only surviving marker is the `mc_synthetic_todo_` id prefix (`injection.rs:23`, `:139`), which nothing documents as a provenance signal. The three text overlays are better marked and still forgeable, which is F2 |
...
| nudge-b-channel2-retirement-is-caller-asserted | `serializer_profile == "claude-code-anthropic"`, a pending directive, and a `channel2_delivered_id` echoing an id the module issued one pass earlier | **Yes for the mechanism; the arm's production reachability is unresolved.** The profile string is settable in-crate and the mechanism is verified: the delivered-id comparison at `transform.rs:9440-9448`, the TTL at `:9450-9458` (10 minutes, `:111`), the pressure collapse at `:9479`, and the id derivation at `:9505-9513`. **No TypeScript sender in this repository emits that profile** — the shipped one emits `opencode-aisdk` (`rust-mode-transform.ts:1339`) — while `ARCHITECTURE.md:125` describes a Claude Code leg as a real deployment, so the `explicit-config-only` label rests on a deployment question this tree cannot answer. The TTL bounds the damage to one arming cycle, which is the right shape; the concern is that the primary retirement path has no corroboration |
...
**Totals: 22 non-vacuous today, 2 partial, 0 blocked outright.**

That distribution needs naming precisely rather than celebrating, because the
reasons differ from 4d's. 4d reached 22 because its surface is request-shaped: an
argument map, a schema, a response. **4e reaches 22 because its surface is
fixture-shaped:** a `CoreState`, a frozen-unit set, a projection and a request,
all constructible by hand. Eleven records need only seeded state and one pass.
The two `Partial` rows do not cluster on a capability: both terminate in a
question outside the sub-part, one in the 4f codecs (whether a message can carry
a full-drop block followed by two taggable blocks) and one in `mc-store` (whether
the `mc_tags` triggers advance `generation` on delete and update).
...
is worse again: 22 constructible records against a suite no automation executes
...
- **The shared gap, and it is a compliance gap rather than a preference: neither
  record names a marker constant.** METHOD.md requires marker names to be
  constant and globally unique, and unlike 4d — where one of the two `sometimes`
  records supplied a name and the other did not — **neither 4e record supplies
  one**. Give each a constant of the same shape as the table above, for example
  `USER_HINT_FRAGMENT_TRUNCATION_SERVED` and
  `OVERLAY_KINDS_COLLIDED_ON_ONE_BLOCK`, so the assertions stop being anonymous.
- **One refinement on the second record.** Its three-kind conjunct is bounded by
  construction at three, not four: a tool result is ineligible for the temporal
  marker (`:8642-8647` requires an authored user message) and for the user hint
  (`:8789` requires `role == "user"`). Record the maximum alongside the marker,
  or a campaign that never fires the three-way arm cannot be distinguished from
  one where four was the unreachable target.
...
1. **`F0`, running the 277 existing checks in CI at all. This remains the
   ...
   **zero** new records and **protects 277 existing checks**: 237 whole-pass and
   transitive tests plus the 5 `nudge_formula_tests` in `transform.rs`, plus 35
   file-local tests across the six other units. **Nothing else on this list
...
3. **`F7`, frozen-unit seeding, which is the largest single block of value and
   ...
   Between them these make six records non-vacuous with no new infrastructure:
   ...
   `render-a-orphan-tool-arc-has-no-production-detection` (the removal half),
   `nudge-b-frozen-todo-pair-retires-only-on-a-bust`,
   `nudge-b-injected-todo-pair-carries-no-provider-visible-provenance`, and the
   weaker form of `render-a-overlay-targets-stale-indices-after-full-drop-filter`
...
4. **`F3` and `F6`, the two-pass tier, which is the only way three records become
...
5. **`F2`, forged overlay markers, which needs nothing but a hand-built ingress
...
6. **`F5`, a cap actually binding, for the one budget that binds in ordinary
   ...
   a fixture.
7. **`F4`, overlay row growth driven by turns.** No fault, and the oracle is a row
   count. Seventh on cost because the cadence step is
   `max(25_000, 0.08 * tail_tokens)` (`:9623-9627`), so a fixture must simulate
   that much unreduced growth per row to make accumulation visible. It makes
   `nudge-b-channel1-append-rows-have-no-reaper` half valid — the bound half. The
   removal half cannot be written at all until someone states a bound in passes,
   which is a product decision rather than a capability.
8. **`F8` cross-process, the cheap half of the differential.** Two `cargo test`
...
9. **`F8` cross-language, last on cost and first on consequence.** This is the
...
- Whether the nudge bands were calibrated on a tail the Rust metric cannot
  ...
  owner decision rather than a code fix.
...
- Whether the `mc_channel1_appends` reaper should key on the overlay frontier, on
  tag retirement, or on compartment coverage. Until this is answered the removal
  half of that record's check cannot be written, because METHOD.md's liveness rule
  requires the bound stated in passes and no bound exists.
