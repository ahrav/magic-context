...
- The situation has two independent halves, and the first gate reads only the first of
  them. `crates/mc-shm-transport/src/backend/ring.rs:773-779` returns `Ok(None)` on
  ...
  (`ring.rs:948-957`).
- The counter that reaches the cap: incremented at `ring.rs:828`
  (`active_leases.fetch_add(1, Relaxed)`) as part of the same block that stores
  `SLOT_RECEIVER_LEASED` and advances `consumed` (`:825-828`); decremented only at
  `ring.rs:908` (`fetch_sub(1, Relaxed)`) inside `release`, after the
  `SLOT_RECEIVER_LEASED → SLOT_RELEASE_PENDING` compare-exchange at `:886-893`.
...
- Observability. Both halves are visible in one `conservation()` snapshot
  (`ring.rs:913-997`): `descriptors.receiver_leased` for the held set (`:968-977`) and
  `descriptors.published` for the queued backlog (`:948-957`). The marker therefore
...
- Where it is reachable today: only `lease_limited_profile()`
  ...
  `lease_limit_reports_backpressure_then_recovers_after_release` (`:278-293`). That test
...
- Where it is not reachable: the shipped host. `max_leases` is `DESCRIPTOR_DEPTH`,
  which is 8 (`crates/mc-host/src/ring_transport.rs:32`, `:47-50`), and `receive_one`
...
`ring.rs:774` was never taken with a frame pending, so no assertion about resuming after
...
system — the comment at `ring.rs:775-777` states that a full lease set is backpressure
