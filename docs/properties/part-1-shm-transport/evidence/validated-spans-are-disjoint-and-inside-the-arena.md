...
- `crates/mc-shm-transport/src/harness.rs:68` — the fuzz target calls
  `descriptor.validate(identity, MAX_FRAME_BYTES)`. The arena argument is the
  constant floor, never anything larger.
- `crates/mc-shm-transport/src/harness.rs:69-81` — the accept-path assertions:
  body bound, span count, per-span `end <= MAX_FRAME_BYTES`, and `summed ==
  body_len`. No pairwise comparison.
- `crates/mc-shm-transport/tests/contract.rs:73` and `:675` — the two tabled
...
(`packages/mc-shm-native/src/lib.rs:1195-1198`), and `to_vec` would produce a body
...
- Findings: the catalog names three conditions and says "relaxing any one
  ...
  and `:401-409` all resolved at `9c1eb4d1`; at `e447c927` the latter two were
  `:450-456` and `:467-476`, and after the #131 rewrite (verified 2026-08-31)
  the same constructs sit at `:238-244` (summed-length check) and `:252-261`
  (two-span arm). `SpanPlan::reserve` does produce
...
- Conclusion: resolved with answer. Disjointness holds at HEAD as a derived
  consequence; the `spans[1].offset != 0` condition (`:256` at post-#131 HEAD)
  is the single condition whose relaxation also breaks the
