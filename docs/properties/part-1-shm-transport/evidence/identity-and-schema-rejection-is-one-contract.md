...
- `descriptor.rs:199-213` — `schema_version != DESCRIPTOR_SCHEMA_VERSION`
  yields `UnsupportedSchema` (`:199-201`); `identity.sequence == 0` yields
  `InvalidSequence` (`:202-204`); `incarnation != expected.incarnation` yields
  `WrongIncarnation` (`:205-207`); `lane != expected.lane` yields `WrongLane`
  (`:208-210`); `sequence != expected.sequence` yields `InvalidSequence`
  (`:211-213`).
- `backend/sample.rs:77-91` — the same five, same order, same error variants,
  written against `self` instead of a destructured `Self`.

The third reader is narrower. `Ring::release`
(`backend/ring.rs:1175-1247`) checks `is_quarantined` (`:1176-1178`),
`incarnation` against the grant (`:1179-1181`), `lane` against the grant
(`:1182-1184`), `sequence == 0` (`:1186-1188`), and `sequence > consumed`
(`:1194-1196`). It then reads the slot's raw `SharedDescriptor` with
`read_volatile` (`:1201`) and compares three fields directly — `incarnation`
(`:1202-1204`), `lane` (`:1205-1207`), `sequence` (`:1208-1210`). It never calls
...
- `try_receive` (`ring.rs:1055-1137`) calls `shared.snapshot().validate(...)` at
  `:1095` and, on any error, calls `self.enter_quarantine()` at `:1098` before
  returning `RingError::Descriptor`. Terminal.
- `reclaim_completed` (`ring.rs:1470-1566`) calls the identical
  `snapshot().validate(...)` at `:1490-1494` and maps the error straight through
  with `.map_err(RingError::Descriptor)?`. No quarantine. Its only caller is
  `try_reserve` at `ring.rs:916`, which wraps it as
  ...
  close-failure path (`lib.rs:283`, `:290`, `:319`, `:324`, `:345`, `:352`,
  `:381-382`, `:418-419`).
...
`tests/contract.rs:73` `descriptor_rejects_every_untrusted_identity_and_span_failure`
...
`tests/contract.rs:570` covers its schema, length, and wire-header cases.
`tests/ring.rs:128-178` covers wrong incarnation, wrong lane, wrong sequence,
...
The first is drift. A sixth condition is added to `descriptor.rs:199-213` — say
a lane upper bound, or a rejection of a sequence above the published cursor —
and `sample.rs:77-91` is not updated. Both decoders keep passing their own
...
(`ring.rs:1482-1484`). The direction stops making progress with no terminal
...
twice from peer-writable memory — once at `ring.rs:1093` for receive and once at
`:1489` for reclaim — with the whole lease lifetime in between. Any field can
...
- Sources examined: `backend/ring.rs:1055-1137`, `:1175-1247`, `:1470-1566`, `:916`,
  `:1373-1392`; `descriptor.rs:183-284`; `backend/sample.rs:72-115`;
  `crates/mc-host/src/ring_transport.rs` and
  `packages/mc-shm-native/src/lib.rs`, searched for `enter_quarantine`,
  `RingError::Descriptor`, and `ProducerError::Ring`;
  `tests/contract.rs:73`, `:570`; `tests/iceoryx.rs:164-229` (deleted by
  `0f336d3c`);
  `tests/ring.rs:128-178`.
- Findings: no. `enter_quarantine` is called from exactly one place inside the
  transport, `ring.rs:1098` on the receive-validation path. Every other
...
- Missing evidence: whether the reclaim-path rejection is reachable in the
  ...
  between the receive read at `:1093` and the reclaim read at `:1489`, which is
