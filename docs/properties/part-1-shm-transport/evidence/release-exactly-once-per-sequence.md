...
- `crates/mc-shm-transport/src/backend/ring.rs:1434-1450` `slot_ptr` — the index is
  `(sequence - 1) % self.grant.descriptor_depth` (`:1438`). Sequences `N` and
  `N + depth` share one slot, so identity uniqueness across laps is a property of
  the descriptor contents, not of the address.
- `ring.rs:1212-1219` — the single mutation point:
  ```rust
  let changed = unsafe {
  ...
  `active_leases` decrement at `:1229-1234` — is downstream of it, so both inherit its
  exactly-once character.
- `ring.rs:1220-1227` — the error mapping: an observed `SLOT_RELEASE_PENDING` or
  ...
  path.
- `ring.rs:1190-1210` — the checks that run *before* the CAS: a `consumed` acquire load
  (`:1192`) with `sequence > consumed` rejected (`:1194`), then a
  `std::ptr::read_volatile((*slot).descriptor.get())` (`:1201`) and three field
  comparisons against the identity (`:1202`, `:1205`, `:1208`). None of these is atomic
  with the CAS at `:1213`, and the descriptor read is a separate access from the state
  transition.
- `crates/mc-shm-transport/src/lease.rs:184-192` `release_once` — a second,
  independent guard: `if self.released { return Err(LeaseError::DuplicateRelease); }`
  (`:185-187`). So duplicates *through one lease handle* are caught locally without
  ever reaching the ring. Span re-verified at post-#131 HEAD: `:184-192`.
- `lease.rs:201-207` `Drop` — the third release entry point, discarding its result
  at `:204`. A duplicate arriving here is therefore invisible; see
  `release-failure-is-observable`.
- `ring.rs:1470-1566` `reclaim_completed` — after a successful release the producer
  resets `reservation_len`, `completion_sequence`, and `state` (`:1552-1554`) but
...
- Existing checks, corrected and re-anchored at post-#131 HEAD: the
  ...
  `crates/mc-shm-transport/tests/ring.rs:152-175`; `:177-180` is the
  following `Exhausted` assert. It covers wrong incarnation (`:152-159`), wrong lane
  (`:160-167`), wrong sequence (`:168-175`), and duplicate (`:176`).
  `tests/ring.rs:206` `stale_lap_release_cannot_complete_recycled_slot` is confirmed
  and is a genuine full-lap test: it wraps `depth` sequences (`:212-216`), leases a
  fresh frame (`:218`), and asserts the lap-old identity yields `InvalidSequence`
  (`:219-223`).

## Failure scenario

The sequential cases are covered. The uncovered one is the read-then-CAS window,
which needs a second party progressing between `:1199` and `:1210`:
...
2. Party A calls `release(N)`. The quarantine, incarnation, lane, and `consumed`
   checks pass. At `:1199` it reads the descriptor and, because the slot still holds
   sequence `N`'s residual bytes, the three comparisons at `:1200-1208` pass.
3. Before A reaches `:1210`, the legitimate holder of `N` releases, the producer
   runs `reclaim_completed` and frees the slot (`:1554`), then reserves, commits
   sequence `N + depth` into the same slot (`:1615-1620`), and the receiver leases
   it (`:1115`).
4. A's CAS at `:1211-1218` now observes `SLOT_RECEIVER_LEASED` — belonging to
   `N + depth`, not `N` — and succeeds.
5. `completion_sequence` is stored as `N`, not `N + depth` (`:1229-1233`). The next
   `reclaim_completed` compares `completion != next` at `:1482` and breaks, so
   reclamation stalls; meanwhile `active_leases` has been decremented for a lease
   that is still live (`:1234`).
...
`ring.rs:1201` and the compare-exchange at `:1213` — a handful of loads and branches,
...
dependency; no platform gating, though the `Acquire` failure ordering at `:1217` and
the `Relaxed` `active_leases` operations at `:1117` and `:1234` mean a weakly-ordered
...
descriptor read at `ring.rs:1201` (fault class F3, absent today), holding party A
...
### Q: Is the descriptor re-read at `:1199` atomic with the arbitrating CAS at `:1210`, and does that gap admit a second successful release for one sequence?
...
- Sources examined: `ring.rs:1175-1247` (`release` in full), `:1434-1450`
  (`slot_ptr` indexing), `:1470-1566` (`reclaim_completed`, including which slot
  fields are reset at `:1552-1554`), `:1594-1620` (`commit_reservation`'s descriptor
  write), `crates/mc-shm-transport/src/lease.rs:184-207`,
  `crates/mc-shm-transport/tests/ring.rs:127-232` (both existing release tests).
- Findings: the two accesses are separate and nothing serializes them. The
  ...
  *misattributed* win: a caller whose pre-checks validated sequence `N` completing
  ...
  inside the window, which is why the existing full-lap test at `tests/ring.rs:206`
  does not catch it — that test is single-threaded, so the recycle completes long
  before the stale release is attempted and the pre-check at `:1206` correctly
  rejects it.
- Missing evidence: none about the code. What is missing is a way to run it — there
  is no failpoint after `:1199`, and the repository has no loom, Shuttle, Miri, or
