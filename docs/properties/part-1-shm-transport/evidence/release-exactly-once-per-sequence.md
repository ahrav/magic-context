...
- `crates/mc-shm-transport/src/backend/ring.rs:1070-1086` `slot_ptr` — the index is
  `(sequence - 1) % self.grant.descriptor_depth` (`:1074`). Sequences `N` and
  `N + depth` share one slot, so identity uniqueness across laps is a property of
  the descriptor contents, not of the address.
- `ring.rs:884-891` — the single mutation point:
  ```rust
  let changed = unsafe {
  ...
  `active_leases` decrement at `:902-906` — is downstream of it, so both inherit its
  exactly-once character.
- `ring.rs:892-900` — the error mapping: an observed `SLOT_RELEASE_PENDING` or
  ...
  path.
- `ring.rs:865-882` — the checks that run *before* the CAS: a `consumed` acquire load
  (`:865`) with `sequence > consumed` rejected (`:866`), then a
  `std::ptr::read_volatile((*slot).descriptor.get())` (`:873`) and three field
  comparisons against the identity (`:874`, `:877`, `:880`). None of these is atomic
  with the CAS at `:884`, and the descriptor read is a separate access from the state
  transition.
...
- `ring.rs:1106-1152` `reclaim_completed` — after a successful release the producer
  resets `reservation_len`, `completion_sequence`, and `state` (`:1144-1146`) but
...
- Existing checks, corrected: the sequential-case ladder is
  `crates/mc-shm-transport/tests/ring.rs:173-197`, not `:174-199` — `:198-201` is the
  following `Exhausted` assert. It covers wrong incarnation (`:174-180`), wrong lane
  (`:181-188`), wrong sequence (`:189-196`), and duplicate (`:197`).
  `tests/ring.rs:222` `stale_lap_release_cannot_complete_recycled_slot` is confirmed
  and is a genuine full-lap test: it wraps `depth` sequences (`:226-231`), leases a
  fresh frame (`:234`), and asserts the lap-old identity yields `InvalidSequence`
  (`:235-239`).
...
`ring.rs:873` and the compare-exchange at `:884` — a handful of loads and branches,
...
dependency; no platform gating, though the `Acquire` failure ordering at `:889` and
the `Relaxed` `active_leases` operations at `:826` and `:906` mean a weakly-ordered
...
descriptor read at `ring.rs:873` (fault class F3, absent today), holding party A
...
- Sources examined: `ring.rs:847-909` (`release` in full), `:1070-1086`
  (`slot_ptr` indexing), `:1106-1152` (`reclaim_completed`, including which slot
  fields are reset at `:1144-1146`), `:1196-1208` (`commit_reservation`'s descriptor
  write), `crates/mc-shm-transport/src/lease.rs:198-221`,
  `crates/mc-shm-transport/tests/ring.rs:148-253` (both existing release tests).
- Findings: the two accesses are separate and nothing serializes them. The
  ...
  *misattributed* win: a caller whose pre-checks validated sequence `N` completing
  ...
  inside the window, which is why the existing full-lap test at `tests/ring.rs:222`
