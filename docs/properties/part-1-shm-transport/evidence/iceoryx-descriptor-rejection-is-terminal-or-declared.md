...
- The five named guarantees, checked one at a time against
  ...
  - **Conservation reporting — absent.** `ring.rs:914-997` reports per-slot
    ...
    (`crates/mc-host/src/provider_recovery.rs:530`, deleted by `ed487e11`, which
    left the snapshot with no host consumer). The iceoryx backend
...
- `crates/mc-shm-transport/Cargo.toml:8-10` at `9c1eb4d1` — `default = ["iceoryx"]`
...
- Sources examined: `backend/mod.rs:1-9` in full; `backend/iceoryx.rs` in full,
  ...
  `crates/mc-host/src/provider_recovery.rs:526-533` at `9c1eb4d1`, deleted by
  `ed487e11`;
