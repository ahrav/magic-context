...
- `crates/mc-shm-transport/src/backend/ring.rs:686-701` — the derivation and the
  claim. `let sequence = published.checked_add(1)` (`:686-688`, corrected from the
  catalog's `:687-701`, whose start line lands on the `.checked_add` continuation
  rather than the `let`), then `slot_ptr(sequence)` (`:689`) and
  `compare_exchange(SLOT_FREE, SLOT_PRODUCER_RESERVED, AcqRel, Acquire)` with
  `.map_err(|_| ProducerError::Exhausted)?` (`:691-701`). A losing CAS is reported as
  `Exhausted` — a backpressure code — regardless of *why* the slot was not free.
- `ring.rs:677` — `published` is loaded `Relaxed` from the shared producer page, so
  it is the surviving process's own view of a cursor the dead process last wrote.
  Nothing else feeds the derivation.
- `ring.rs:1154-1162` `abort_reservation` — corrected span; the catalog records
  `:1154-1162` and the prompt's `1150-1165` is wider than the function. It stores
  `reservation_len = 0` (`:1158`) and `state = SLOT_FREE` (`:1159`).
- **Correction to the catalog record.** The catalog says `abort_reservation` "is the
  ...
  `try_reserve`'s own rollback at `:710` and `:715`, which fire on arena exhaustion
  and arena-planning errors *before* the handle is returned, and
  `reclaim_completed` at `:1146`, which frees a slot from `RELEASE_PENDING`. Neither
  helps here: the rollback path is already past, and reclaim only acts on released
  frames.
- `ring.rs:1397-1404` `impl Drop for ProducerReservation` — `if !self.finished { self.ring.abort_reservation(self.sequence); }`. This is the path a kill skips.
- `ring.rs:1338-1349` and `:1352-1380` — the other `abort_reservation` callers, all
  in-process: `write` on overflow (`:1343`), `commit` on
  `CommitOutsideReservation`, `Underfill`, and `commit_reservation` failure (`:1357`,
  `:1362`, `:1375`), and `abort` (`:1385`).
- `ring.rs:683-685` and `:753` — the symptoms. Once `published - completed` reaches
  ...
  backpressure. `enter_quarantine` (`:1033-1038`) is never called on either path.
- `ring.rs:928-945` — why the accounting looks healthy: `conservation()` counts a
...
- `crates/mc-host/src/shm_provider.rs:645-648` — worth recording as the *negative*
...
1. A producer calls `try_reserve`. `published` is `N-1`, so `sequence = N`, and the
   slot for `N` moves `FREE → PRODUCER_RESERVED` (`ring.rs:691-701`).
...
4. Any later producer on the same object derives `sequence = published + 1 = N`
   again (`:686-688`), and its CAS at `:691-701` fails against the stranded slot,
   returning `Exhausted`.
5. `reserve_until` retries until the deadline and returns `Deadline` (`:753`).
...
`published.store(Release)` at `:1207` or `abort_reservation` at `:1159`. In the
shipped host that span contains the entire serialization of the frame body —
`publish_direct` runs `direct.serialize` inside it
(`crates/mc-host/src/shm_provider.rs:671-675`) and `publish_owned` performs two
...
reservation across `fill.call(views)` (`packages/mc-shm-native/src/lib.rs:679`), and
the two-phase `reserve`/`commit_reservation` pair holds it across an entire return to
JavaScript and back (`:727-733` to `:810-812`). That second shape is the practical
...
(`ring.rs:1417-1426`) with a fresh random incarnation (`:559`), so a replacement peer
...
suspension point (`crates/mc-host/src/shm_provider.rs:747-759`). So the new scenario
...
- Sources examined: `ring.rs:662-734` (`try_reserve`), `:1154-1162`
  (`abort_reservation`), `:1338-1404` (all `abort_reservation` callers including
  `Drop`), `:1417-1426` (`DuplexRing::create`), `:536-562` (`create_in` and the
  random incarnation), `:928-1000` (`conservation`);
  `crates/mc-host/src/shm_provider.rs:299-302`, `:645-691`, `:747-759`;
  `packages/mc-shm-native/src/lib.rs:636-703`, `:705-760`, `:790-815`;
