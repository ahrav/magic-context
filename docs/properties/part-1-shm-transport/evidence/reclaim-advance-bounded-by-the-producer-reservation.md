...
- `:1470-1566` — `reclaim_completed`, the whole loop. It runs on the producer and
  walks completions in strict publication order.
- `:1481-1484` — the loop advances only while `completion_sequence == next`, so
  reclamation head-of-line blocks behind the lowest unreleased sequence.
- `:1485-1487` — the slot must be `SLOT_RELEASE_PENDING`, else
  `InvalidSharedState`.
- `:1489` — `let descriptor = unsafe { std::ptr::read_volatile((*slot).descriptor.get()) };`
  This is a *second* read of the same descriptor. The first was at `:1093` during
  `try_receive`, in the other process.
- `:1490-1494` — the re-read descriptor is re-validated with
...
- `:1495-1500` — the only ordering guard: `validated.allocation_start() !=
  reclaimed` rejects as `InvalidSharedState`. This pins where the advance starts.
- `:1501-1503` — the advance distance: each accepted slot's
  `validated.allocation_len()` accumulates into `run_len`, which is applied as
  `new_reclaimed = reclaimed.checked_add(run_len)` at `:1509-1511`.
  This is the distance, taken from the re-read record. Nothing compares it to what
  the producer reserved.
- `:122` — `reservation_len: AtomicU64` in `DescriptorSlot`.
- `:963-966` — the producer writes it: `(*slot).reservation_len.store(plan.allocation_len(), Ordering::Relaxed)`
  at reservation time, so the value is the producer's own reservation length.
- `:1271` — the only read of `reservation_len` anywhere: inside `conservation()`,
  for the per-state byte tally. `reclaim_completed` never reads it.
- `:1552` and `:1571` — the only other writes, both stores of `0` during reclaim
  and abort.
- `:321` and `:342` — `Mapping::create` and `Mapping::attach` both map the whole
  object `PROT_READ | PROT_WRITE`.
- `:2131` — required seals are `F_SEAL_GROW | F_SEAL_SHRINK | F_SEAL_SEAL`. No
...
and keeping the record internally consistent so `:1490-1494` still accepts it.
...
against the arena in `lease_span` (`:1452-1468`).

## Timing windows and dependencies

The window opens when the receiver's `release` stores `completion_sequence`
(`:1229-1233`) and closes when the producer's `reclaim_completed` reads the slot
(`:1489`). Nothing bounds it: reclamation runs only when the producer next
...
`:1490-1494` being as strong as claimed.
...
advances by the length the producer stored at `:966`, or that reclaim rejects. A
...
### Q: Was the atomic `reservation_len` (`ring.rs:122`) intended to be the producer's trusted record? It is written but never read by `reclaim_completed`. A producer-local table would be trustworthy; is that feasible given `Ring` is thread-confined?
...
- Sources examined: `git log -S "reservation_len" -- crates/mc-shm-transport/src/backend/ring.rs`,
  ...
  later change; `ring.rs:1250-1333` for `conservation()`, its only reader; the
  SAFETY comment at `:1270`, "reservation length is atomic and assigned before
  non-free state is observed", which describes an observation contract for a
  *reader of the snapshot*, not a trust contract; `ring.rs:719-727` for `Ring`'s
...
- Findings: the field's single documented purpose is the conservation snapshot.
  ...
  PhantomData<Rc<()>>` (`:726`) and takes `&self` on every operation, so it is
...
- Missing evidence: any statement of design intent for `reservation_len`. Also
  unestablished: whether a peer rewrite can actually get past `:1492-1496` and
  `:1500` together while remaining internally consistent. I constructed the
