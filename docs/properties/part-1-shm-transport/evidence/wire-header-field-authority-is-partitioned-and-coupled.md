...
(`crates/mc-host/src/shm_provider.rs:565-576`). The TCP path duplicates that rule
by hand at `tcp_frame_channel.rs:198`, so it is a third, per-transport copy.
...
1. **`declared_len == body_len` is load-bearing, not cosmetic.** The host charges
   ingress `header.len as usize` (`shm_provider.rs:580`) and then copies
...
(`backend/ring.rs:1169-1185`, `ProducerError::WireHeaderMismatch`), and a
producer may replace the header freely until commit
(`ring.rs:1326-1335`). So the transport's rule is symmetric and narrow in both
directions: two fields, never bytes 5 through 20.

## Failure scenario

Drop coupling 1 — relax `descriptor.rs:420` to accept any `declared_len` — and
...
zero permits (`:580`), and `lease.to_vec()` then allocates and copies 64 MiB
...
cheap because both numbers are already in scope at `shm_provider.rs:612-617`.
...
- Sources examined: `descriptor.rs:332-423` in full; `wire.rs:306-368` in full;
  `frame_channel.rs:58-76` in full; `shm_provider.rs:562-576`.
