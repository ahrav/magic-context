# Part 4f lens C2: existing-check inventory

Scope: sub-part 4f, "Pure decision units, harness codecs, and config". This lens
is the check inventory only. Claims and configuration belong to a sibling lens,
so no claims register appears here.
...
Production line counts re-derived at `HEAD`: `codec/mod.rs` 299,
...
### In-crate tests (clustered, counts, line ranges, attribution method, reconciliation with 4b and 4e)

**Headline. 192 in-crate tests reach full scope-map 4f: 153 file-local tests in
the ten 4f files plus 39 of the 280 tests in `transform.rs`'s flat `mod tests`.
Restricted to the brief's named files the figure is 164, being 146 file-local
plus 18. `codec/sidecar.rs` has zero tests. There is no integration test in
scope, and none of the 192 runs in CI.**

#### Attribution method, stated so it can be reproduced

File-local tests were counted directly, not derived: every line matching
`#[test]` or `#[tokio::test]` per file, then each attribute resolved forward to
...
sibling attribute such as `#[ignore]`.
...
1. **Enumerate.** All `#[test]` and `#[tokio::test]` attributes: **285** total,
   **280** at or after the flat module `pub(crate) mod tests` (`:12626`), so **5**
   in `mod nudge_formula_tests`. These are exactly 4b's and 4e's figures.
2. **Resolve.** Each attribute resolved forward to its `fn` line, giving 280 test
   ...
   at `:29425`. Both re-derived at `HEAD`; both match 4b and 4e.
...
- A hit counts as **behaviour** only when it is a call or a module constant. A
  bare type mention is tracked separately, for the reason in the next paragraph.
...
The tiers **bracket** rather than pin:
...
reach count is approximately the whole driver population, because 4f is the
decision layer a pass cannot avoid. It is stated so a later pass does not
recompute it and treat it as a finding.

#### File-local tests, clustered

Counted directly at `HEAD`. No `#[ignore]`, no `should_panic`, and no
`cfg`-gated test outside `codec/opencode.rs:2077` exists in any 4f file.

| File | Test module | Tests | Line range of test `fn`s | What they cover |
| --- | --- | --- | --- | --- |
| `selection.rs` | `:1413` | **37** | `:1573`-`:3349` | `:1732` the TS selector golden; `:1858`-`:2486` band, force, emergency and supersession batching; `:2537` the `region_hint` UTF-16 cap and surrogate back-off; `:2552`-`:2762` the `provider_executed`, `frozen_arc` and dynamic-protection filters; `:2836` drop beats edit marker; `:2885` payload purity; `:3107`-`:3300` the duplicate-safe-tool family; `:3349` defer produces nothing |
| `boundary.rs` | `:1975` | **29** | `:2174`-`:3042` | `:2174` constants match TS; `:2247`, `:2285`, `:2329` the three TS goldens; `:2473`-`:2772` the backward reasoning fence and fold-only guard; `:2809`-`:2877` wrap-up watermarks; `:2911`, `:2924` determinism and anchor monotonicity; `:2966`-`:3042` trigger and ordinal-zero edges |
| `config.rs` | `:806` | **26** | `:721`-`:1191` | `:760` the shared TS cache-TTL vectors; `:797`, `:811`, `:829`, `:876`, `:913`, `:930`, `:981`, `:1096`, `:1166` the per-leaf trust tiering; `:837`, `:843`, `:1088` TS schema agreement; `:999`-`:1051` guidance override resolution; `:1181`, `:1191` JSONC stripping and the mtime cache |
| `codec/opencode.rs` | `:1323` | **17** | `:1413`-`:2157` | `:1413`-`:1978` fresh-part completeness, adjacency deletion, native-extras survival, polarity round-trip, reasoning exemptions; `:1860` compaction as boundary; `:2079` the duplicate-id guard, debug-only; `:2113` incremental sidecar pins; `:2157` typed wire projection |
...
carries `#[cfg(test)]` at `:268` and `:715` before its module at `:805-806`;
...
**4f does draw from that module, so the reconciliation the brief asks for is
required rather than vacuous.** The 39, by the 4f unit they name:
...
in `transform.rs`'s flat module**, which is worth stating: the boundary unit's
only in-crate evidence is its own 29 file-local tests.

#### Reconciliation with 4b's 226 and 4e's 237
...
| --- | --- | --- |
| Inside 4b's driver set | **24** | Inside the 210. Already counted in **both** 4b's 226 and 4e's 237. 4f adds no new test here; this is triple-shared evidence |
...
| Outside both | **9** | `:12923`, `:15064`, `:15086`, `:18700`, `:21462`, `:25459`, `:25525`, `:25655`, `:25695`. The only candidates for attribution neither sibling made |

**Consequences, stated so no reader double-counts.**
...
- **The 9 must lie inside 4b's 70 non-driver tests** (`16 + 22 + 5 + 27`).
  Whether each sits in the 27 unclassified bucket or in a helper bucket cannot be
  determined from 4b's file, which does not enumerate its buckets and states at
  `:79` that "the 27 unclassified tests were not hand-read". Recorded as an open
  question rather than resolved. Two of the 9 do match 4b's description of the 27
  as "small serde, timing, geometry and TTL unit tests": `:12923` is a
  context-limit geometry unit test and `:12704` is a TTL unit test, though
  `:12704` is a driver.
...
No correction is issued: the edge is in how a driver call is spelled, not in
what counts as a driver, and it moves the 24/15 split above by at most a few
tests. It is recorded because three sibling documents now report three
different numbers for the same set.

#### `#[ignore]`, `should_panic`, and property tooling

**`#[ignore]`: none found in any 4f file.** Verified by matching both `#[ignore]`
and `#[ignore = "..."]`, the form 4e found a bare grep misses. The 4f draw from
`transform.rs` does include one ignored test, `:28388`
...
oracle, and it is written a way a `should_panic` grep misses:
`codec/opencode.rs:2079`
`whole_array_tool_use_guard_rejects_both_id_collision_directions` uses
`std::panic::catch_unwind` at `:2106` and asserts `.is_err()`. It is the only
`catch_unwind` in 4f. See the guards section for why it carries
`#[cfg(debug_assertions)]` at `:2077`.
...
5. **`scripts/test-rust.sh` would cover 4f and no workflow invokes it.** The
   ...
   `package.json` as `test:rust` and into `check:all`. Neither name appears in
   any workflow; the single grep hit is a comment at `ci.yml:378` that mentions
   `check:all` while describing what a contributor's local pass covers.

#### `release_contract_conformance.rs`: it does not run, and its own header says it must

**Established: it does not run in CI.** The binary is not selected by
`--test lifecycle_cli`, no other workflow step names it, and no `--workspace`
Rust test job exists. Its three tests are at `:16`
`credential_constants_match_the_release_contract`, `:47` and `:129`.

Its header argues the opposite is required. `:1-8` states that each equality is
"load-bearing at runtime", that "a drifted fingerprint label mismatches every
presented credential fingerprint, and a drifted canonical encoding makes every
qualified closure digest unverifiable at spawn", and concludes "so the drift must
fail the build, not the deployment". **The drift fails no build.** The scope map
already raised this as needing human input (`:681`); this pass confirms the
mechanism rather than resolving the question.
...
test && bun run --cwd packages/cli test && bun run --cwd packages/retina-local-fs
test`. `ci.yml:317` runs the pi-plugin suite again directly. `bun test` from a
package root recursively discovers every `*.test.ts` beneath it, so the gates
below do run on every pull request.
...
is the TypeScript one and the Rust leg runs nowhere.** For every other
cross-language fixture in 4f the pattern is worse still: the fixture is
...
**No 4f fixture has a provenance guard, and no workflow or npm script
regenerates any of them.** This is the one place 4f is strictly weaker than 4e:
4e at least has `tail_hygiene.rs:1035-1039`, an `assert_eq!` recomputing the
fixture input hash against a `provenance.input_sha256` field, with a dedicated
mutation test at `:1099`. Verified here that no 4f file contains any
`provenance`, `input_sha256` or `generator_version` fixture assertion; the only
`provenance` matches in 4f are `config.rs`'s production `CacheTtlProvenance`
enum (`:151`, `:159-166`, `:204`) and its test at `:745`, which concern the
origin of a config value, not of a fixture. The two codec goldens do carry
`generated_from` and `projection_oracle` top-level fields, and neither is
deserialized: `codec/mod.rs`'s `OpenCodeGolden` (`:28-34`) and `PiGolden`
(`:41-47`) declare only `coverage`, `missing_capture_classes` and `cases`. Both
fields are inert documentation. A 4f fixture that no longer matches the
generator it came from is caught by review alone.
...
stricter model-selection policy than the current TypeScript implementation".
Six further comments assert TypeScript agreement (`:24`, `:35`, `:38`, `:41`),
and three Rust tests check named schema values (`:837`, `:843`, `:1088`). No
TypeScript test parses `config.rs`, and the divergence itself, being a
deliberate inequality, is asserted by nothing on either side. `CONFIGURATION.md`
(59,329 bytes at the repository root) is the documented contract.

### Production assertions and guards (clustered)

Measured over production halves only, meaning each file up to its test-module
line as listed in the file-local table, with the pre-module `#[cfg(test)]`
regions at `config.rs:268`, `:715` and `boundary.rs:737`, `:1009` noted as
test-only.

**Runtime assertions: three, all `debug_assert!`, all compiled out of release,
all in one file.** Verified by matching `debug_assert` across every 4f
production half, which returns exactly these three lines and nothing in the
other ten files.

| Site | Guard | Release behaviour | Test |
| --- | --- | --- | --- |
| `codec/opencode.rs:251` | `debug_assert!(replace_from <= messages.len())` | **Still panics, elsewhere.** The slice `&messages[replace_from..]` at `:258` is an index-out-of-range panic on the same condition. The assertion is a better message for a failure that is not silent | none |
...
| `codec/opencode.rs:466` | `debug_assert!(duplicates.is_empty(), "OpenCode serialization produced duplicate tool_use ids: {duplicates:?}")`, inside `assert_unique_tool_use_ids` (`:462-470`) | **No enforcement at all.** The whole function becomes a no-op, though `duplicate_tool_use_locations(messages)` at `:465` still runs and its result is discarded | `:2079`, debug-only |

**The brief's premise needs one correction, and it sharpens the finding rather
than softening it.** `codec/opencode.rs:251` is not the dangerous one. Its
condition is re-checked by the language at `:258`, so a violated `:251` fails
loudly in every profile. **`:252` is the dangerous one**: same function, adjacent
line, no release equivalent, and `take` at `:265` converts a violated
precondition into a silently truncated sidecar. Both were re-read at `HEAD`.
Neither has a test: `decode_opencode_sidecar_incremental` has exactly one test,
`incremental_sidecar_carries_pins_across_three_generations`
(`codec/opencode.rs:2113`), which calls it three times with `replace_from` of 1,
2 and 2 (`:2128`, `:2139`, `:2151`), all in range. Its single production caller
is `lib.rs:12578`, which is outside 4f.
...
`lib.rs:21308`. Its only test, `:2079`, carries `#[cfg(debug_assertions)]` at
`:2077`, so it does not compile in a release test build, and **verified: there is
no `cfg(not(debug_assertions))` anywhere in 4f**, so no release-arm counterpart
exists. Whichever profile ships, the guard's shipped behaviour has no test.

**Zero unconditional runtime assertions in the 4f production halves.** Verified
by matching `assert!`, `assert_eq!` and `assert_ne!` excluding `debug_assert`
across every production half: no match in any of the eleven files. **Zero
compile-time `const _` assertions in scope.** This matches 4e's finding for its
own scope.

**Panicking sites: two, and 4f is therefore unlike 4e, which had none.**
...
is a UTF-8 boundary invariant on a hand-rolled cursor walk in a file with one
test.
...
**Both harness directions have a golden and both goldens are one case each,
their oracle is the input itself, and each declares one required block class as
missing so the coverage gate passes without it.**

| Codec path | Golden | Case count | Oracle |
| --- | --- | --- | --- |
...
| `decode_opencode_with_sidecar`, `decode_opencode_with_sidecar_and_base`, `decode_pi_with_sidecar` | **none** | — | Re-exported at `codec/mod.rs:6` and `:10`; not imported by the golden module, whose `use super` list is `:24-26` |
| `encode_opencode_with_session_exemptions` | **none** | — | Re-exported at `codec/mod.rs:7`, absent from `:24-26` |
| `decode_opencode_sidecar_incremental` | **none** | — | Hand-built only, `codec/opencode.rs:2113` |
| `codec/sidecar.rs`: `stamp_block_identity` (`:158`), `decoded_block_fingerprint` (`:151`), `block_is_unchanged` (`:192`) | **none** | — | No direct test anywhere. Reached only transitively from `codec/opencode.rs:553-554`, `:742`, `:763` and `codec/pi.rs:303-304`, `:372`, `:463`, so their only exercise is the two one-case goldens |
| Compaction extraction, both harnesses | **excluded by construction** | — | `strip_opencode_compaction` (`:273-281`) and `strip_pi_compaction` (`:283-288`) remove compaction from the expected value before comparison. Covered by hand-built tests only: `codec/opencode.rs:1860`, `codec/pi.rs:1487` |

**The scope map's suspicion about the coverage manifest is confirmed, and the
answer is that the gate is designed to pass without the class.** The scope map
asked "whether the `coverage` and `missing_capture_classes` manifest in
`codec/mod.rs`'s golden test admits an unclassified block shape silently"
(`:641-644`). `assert_coverage_or_recorded_missing` (`:254-271`) fails only on a
required class that is in neither list (`:262-266`). Listing a required class in
`missing_capture_classes` clears it. Reading both fixtures at `HEAD`:
...
**The encode-direction oracle is self-referential, and it should be recorded as a
sampling limit rather than as coverage.** For both harnesses the expected value
is a transformation of the test's own input, not an independently generated
expected output. That proves round-trip identity over unmodified content. It
cannot detect a decode error that the encoder symmetrically reverses, because
both halves are the code under test. The `projection_oracle` field in each
fixture names an intended external oracle and, as recorded above, is never
deserialized.
...
1. **`codec/sidecar.rs` is the only file in 4f with zero tests, and it owns the
   ...
   no `mod tests`, no test attribute. `stamp_block_identity` (`:158`),
   `decoded_block_fingerprint` (`:151`) and `block_is_unchanged` (`:192`) have no
   direct test anywhere in the crate; the only other named members,
   `has_stamped_block_identity` (`:188`), `match_block_metas` (`:229`),
   `stable_hash` (`:292`) and `stable_hash_prefix` (`:298`), have none either.
   Every exercise they get is transitive, through
   `codec/opencode.rs:553-554`, `:742`, `:763` and `codec/pi.rs:303-304`,
   `:372`, `:463`, which means through the two one-case goldens described above.
   `block_is_unchanged` decides whether a decoded block is treated as unchanged
   and therefore whether native extras replay verbatim; a fingerprint that
   silently collides or silently differs changes served bytes. The scope map
   predicted this exactly, calling it "the untested block-identity stamper"
   (`:622`, `:639-641`). Confirmed, and it is the quietest area in the sub-part.

2. **`caveman.rs` is 651 lines behind one snapshot test, and 4f cannot see the
   consumer 4e found.** `differential_golden_matches_typescript_oracle`
   (`:626`, extent `:626-650`) replays 42 frozen cases from
   `caveman-golden.json` (`:628`) and is the entire check on the file. The
   fixture is generated from `caveman.ts`, has no TypeScript consumer, and no
   workflow regenerates it, so the compatibility contract is a snapshot rather
   than the live oracle. The file also carries 17 of the 26 production
...
3. **The codec goldens are one case each, their gate is built to pass without a
   ...
   from the input by `strip_opencode_compaction` / `strip_pi_compaction`. Six
   further exported codec entry points have no golden at all, and compaction
   extraction is excluded from the oracle by construction. These two tests carry
   3,685 lines of `codec/opencode.rs` plus `codec/pi.rs` production code.

Further quiet areas, below the three the brief asks for:
...
7. **`selection.rs:1258` is a live production `unreachable!` whose safety rests on
   a non-local early return.** `PassClass::Defer => unreachable!("defer returned
   early")`. 4e recorded zero panicking sites in its 9,304 lines; 4f has this one
   plus the infallible-by-construction `scheduler.rs:875`.
...
9. **The one two-legged cross-language fixture is gated only on the leg that is
   not this crate.** `cache-ttl-routing-vectors.json` has 5 cases. The
   TypeScript leg (`prompt-surface.test.ts:105`) runs on every pull request via
   `ci.yml:257`; the Rust leg (`config.rs:760`) runs nowhere. A Rust-side
   `resolve_cache_ttl` regression is invisible to CI.

10. **No 4f fixture has a provenance guard and no workflow regenerates any of
    them.** Eight fixtures, zero hash checks, and the two `generated_from` /
    `projection_oracle` fields that exist are not deserialized
    (`codec/mod.rs:28-34`, `:41-47`). 4e at least has
    `tail_hygiene.rs:1035-1039` with a mutation test at `:1099`, even though it
    runs on no CI machine.
...
11. **`config.rs`'s per-leaf trust tiering is the crate's only security-shaped
    ...
    `:930`, `:981`, `:1096`, `:1166`), which is the densest per-leaf coverage in
    4f. But `:8`'s claim that the module "intentionally keeps stricter
    model-selection policy than the current TypeScript implementation" is an
    inequality no test on either side evaluates, and no TypeScript test parses
    `config.rs`.
...
12. **No integration coverage of the decision layer, the codecs, or the trust
    ...
    and 4d's ten. Two of the seven have names that suggest otherwise and do not
...
- Which of 4b's five buckets hold the 9 tests 4f attributes and neither sibling
  did (`:12923`, `:15064`, `:15086`, `:18700`, `:21462`, `:25459`, `:25525`,
  `:25655`, `:25695`)? 4b's file does not enumerate its buckets and states at
  `:79` that the 27 unclassified were not hand-read, so the union of the three
  parts over `transform.rs` can only be bracketed at 253 to 262 of 280.
  Unresolved, needs 4b's per-test bucket assignment.
- Does `caveman.rs` belong to 4e or 4f? The scope map says 4e (`:590`), 4e's
  inventory counts its test, and this brief assigns it to 4f. One test and 651
  production lines are currently double-counted. (needs human input)
...
- Is the encode-direction oracle at `codec/mod.rs:88` and `:211` intended to be
  ...
  carries a `projection_oracle` field that nothing deserializes, which suggests
  the latter. Unresolved.
- Should a `debug_assert!` whose condition is independently enforced by the
  language in release (`codec/opencode.rs:251` against the slice at `:258`) be
...
- Should `release_contract_conformance.rs` be inventoried by 4f at all? The brief
  names it, and its content reaches no 4f file. Its three tests remain
  un-run by any workflow while its header (`:1-8`) argues the drift "must fail
  the build". The scope map raised this at `:681`. (needs human input)
- Does a test that never runs in CI count as `Exercised: partial` or
  `Exercised: not yet`? This governs all 192 checks above. 4b, 4c, 4d, 4e, the
  scope map (`:681`) and both 4f sibling lenses have raised it. (needs human
  input)
...
- **The 39-test figure is a symbol match over parsed test bodies, not coverage
  ...
  placement statement here is structural. The 285 attribute count, the 280 plus 5
  split, the first and last `fn` lines, the eleven per-file test counts, the
  eleven scope line counts, the zero `#[ignore]` and zero `should_panic` results,
  the three `debug_assert` sites, the two panicking sites, both `ci.yml` line
  numbers at both commits, the two goldens' case counts and coverage arrays, and
  every cited line were obtained directly at `HEAD`.
- **The 4f symbol rule is stated in full above and is reproducible**, which is
  the one respect in which this attribution is stronger than 4e's. A later pass
  recomputing 39 should get 39 from the ten module paths and eight named items
  listed, applied to brace-matched bodies with literals stripped.
- **Three driver detectors disagree: 210 (4b), 207 (4e's lens C), 206
  (reproducing 4b's stated literal rule here), 196 (a transitive helper
  fixpoint).** The 24/15 split of 4f's 39 shifts by a few tests under each. No
  correction is issued to any sibling.
- **The type-mention tier, 87, and the reach tier, 206 to 210, are reported and
  discarded.** Both are artifacts of 4f being the decision layer every pass
  consults, exactly as 4e's 190 was an artifact of 4e being the terminal stage.
- **`caveman.rs`'s single test is counted by both this file and 4e's**, so 4e's
  277 and any 4f figure overlap by exactly one in-crate test plus 651 production
  lines.
- **The production-versus-test boundary in every 4f file is a line number**, and
  four test-only regions sit above the main test module: `config.rs:268`, `:715`
  and `boundary.rs:737`, `:1009`. A guard count keyed on the first `#[cfg(test)]`
  per file would misattribute them.
- **Whether the two codec goldens exercise a given production path was inferred
  ...
  re-export list (`:5-11`), not from execution.** Six exported entry points are
  absent from the former and are recorded as having no golden on that basis.
