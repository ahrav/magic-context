...
**The whole producer.** `crates/mc-host/src/ring_transport.rs:165-179`:
...
**What makes the `Err` arm fire.** `self.accounting()` (`:131-139`) is
`self.admission.snapshot()`. `AdmissionController::snapshot`
(`crates/mc-shm-transport/src/profile.rs:451` post-#131; the file was not fully
re-swept) returns `Err` on exactly one
condition:
...
- `"setup_failure"` in Rust: only `ring_transport.rs:176`.
...
**The counters, and why they do not close the gap.** Post-#131, `diagnostics()`
emits four counters at `:190-193` (`record_attachment` and its `attachment`
field were removed with the eventfd rewrite):
...
**A second degeneracy in the same output, resolved by removal.** Pre-#131,
`record_attachment` and `record_activation` were called back to back with no
branch between them, so `attachment.completed` and `activation.completed` were
equal for all time. PR #131 removed `record_attachment` and the `attachment`
field outright, which retires that degeneracy; the doc claims this paragraph
cited (`docs/mc-host-shm-transport.md:66`, `:26-36`) were not re-checked in
this pass and may describe the removed field. Kept as history rather than
restated as a live finding.
...
- `ring-a-host-never-quarantines-an-admission-charge` explains why the
  `"quarantined"` field inside the healthy arm (`:171`) is structurally zero.
...
Taken together, the degenerate fields post-#131 are: `quarantined` is always
zero, and
`state` is insensitive to every failure the host actually experiences. (The
`attachment`-equals-`activation` degeneracy retired with the field's removal.)
...
Existing checks: `ring_transport.rs:858-897`
...
fields, the bounds, both accounting figures, and the four remaining counters,
plus a seven-string redaction sweep at `:886-896`. It is a thorough test of the healthy
...
- Sources examined: `docs/mc-host-shm-transport.md:51-73` (the whole Doctor and
  diagnostics section), `ring_transport.rs:142-196`,
...
- Sources examined: `ring_transport.rs:165-179` (the `match`), `:190-193` (the
  counters), `:224` (the only `exhaustions` increment),
...
- Findings: `resource_exhaustion` being a *terminal* class is the tension.
  ...
  health vocabulary exists elsewhere in
  the crate.
