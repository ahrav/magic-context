...
Epoch synapse-tail-881be45b. Adapted from the af8ef126 analyzer: connection
loss is surfaced as its own structured count, and the A/A interpretation is
computed from the epoch's own pairing and ratio results instead of being
carried over.
"""
...
def invalid_reason(row: dict) -> str | None:
    ...
    if summary.get("connection_loss_errors"):
...
def flat_row(row: dict) -> dict:
    ...
    result = {
        ...
        "connection_loss_errors": summary.get("connection_loss_errors", 0),
    }
...
def aa_comparisons(rows: list[dict], required: int) -> dict:
    ...
    schedule_validity = "PASS" if valid_count == required else (
        f"PARTIAL: {valid_count}/{required} valid positions"
    )
    result = {
        ...
        "interpretation": "computed from this epoch's pairing and ratio ranges; see report.md",
    }
...
def analyze_phase(phase: str) -> dict:
    ...
    for status_path in sorted((ROOT / phase / "raw").glob("*.status.json")):
        ...
        valid = (
            ...
            and not summary.get("connection_loss_errors")
...
def main() -> None:
    ...
    aa_mechanical = aa_comparisons(aa["rows"], aa["required"])
