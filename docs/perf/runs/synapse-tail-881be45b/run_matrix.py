...
Epoch synapse-tail-881be45b. Adapted from the af8ef126 epoch runner with two
changes: the pinned artifact hash matches the 881be45b release build, and the
runner samples /proc/loadavg immediately before every phase block, appending
the sample to environment.txt and block_loadavg.ndjson (the af8ef126 manifest
flagged the missing per-block load capture).
"""
...
ARTIFACT_SHA256 = "304571f903697540689d755497f098b1db25cd6f7511868c5801abb76665aa66"
DELAYS = (0, 5, 25)
...
def sample_block_load(phase: str, block: int, sequence: int) -> None:
    """Record /proc/loadavg immediately before the first cell of a block."""
    ...
    with (ROOT / "block_loadavg.ndjson").open("a") as sink:
        sink.write(json.dumps(entry) + "\n")
    with (ROOT / "environment.txt").open("a") as sink:
        sink.write(
            f"block_loadavg phase={phase} block={block} "
            f"sequence_start={sequence} utc={entry['utc']} loadavg={loadavg}\n"
        )
...
def calibration() -> dict:
    ...
    for index, cell in enumerate(cells, 1):
        if cell["block"] not in seen_blocks:
            seen_blocks.add(cell["block"])
            sample_block_load("calibration", cell["block"], index)
...
def run_schedule(phase: str, schedule: list[dict], max_cells: int | None) -> None:
    ...
    for sequence, cell in enumerate(schedule[:limit], 1):
        status_exists = (ROOT / phase / "raw" / f"{sequence:05d}.status.json").exists()
        if cell["block"] not in seen_blocks:
            seen_blocks.add(cell["block"])
            if not status_exists:
                sample_block_load(phase, cell["block"], sequence)
