...
struct Channel {
    ...
    next_lease: u32,
    closed: bool,
    // `_reservation` releases the process-wide claim when `Channel` drops.
    // Releasing the process-wide claim when the channel entry is removed keeps quarantined and alias-holding entries reserved.
    // Quarantined and alias-holding entries remain reserved while their mappings live.
...
impl GrantReservation {
    /// `GrantReservation::claim` atomically claims both lane grants.
    /// A grant already claimed anywhere in the process indicates a replayed or duplicated descriptor.
    #[cfg(target_os = "linux")]
...
fn decode_hex<const N: usize>(text: &str) -> Result<[u8; N]> {
    ...
    // The parser accepts only lowercase hexadecimal because `from_str_radix` also accepts uppercase and sign prefixes.
        ...
        REGISTRY.with(|registry| {
            ...
            let first = insert_channel(
                ...
                Channel {
                    ...
                    next_lease: 0,
                    closed: false,
                    // Test pairs attach freshly created local rings, never host descriptors, so no process-wide grant is claimed.
                    _reservation: None,
                },
            )?;
