...
describe("shm re-upgrade drain and fencing (bun)", () => {
    test(
        "predecessor retires only at pending-zero with all raw routes closed",
        async () => {
    ...
    test(
        "an in-flight managed call settles on the predecessor and its orphaned route closes at pending-zero",
        async () => {
            const provider = recoveryProvider();
            let allowGrant = false;
            const harness = await connectHarness(
            ...
            try {
                ...
                await waitUntil(
                    () =>
                        conn1.frames.some(
                            (f) => f.ty === PeerFrameType.Goodbye && f.channel === 9,
                        ) &&
                        conn1.frames.some(
                            (f) => f.ty === PeerFrameType.Goodbye && f.channel === 0,
                        ),
    ...
    test(
        "an occupied predecessor slot defers the next promotion without forcing closes",
        async () => {
            const provider = recoveryProvider();
    ...
    test(
        "a stale shadow success racing primary retirement cannot publish and returns its permits",
        async () => {
            ...
            const harness = await connectHarness(
                (peer) => {
                    scriptNegotiations(peer, (index, frame, conn) => {
                        if (index === 0) return tcpSelectionBody("unavailable");
                        // Hold the grant until the test retires the primary.
                        void grantHeld.then(() => {
                            conn.socket.write(
                                encodePeerFrame({
                                    ty: PeerFrameType.Response,
                                    corr: frame.corr,
                                    body: Buffer.from(
                                        JSON.stringify(grantSelectionBody()),
                                        "utf8",
                                    ),
                                }),
                            );
            ...
            try {
                ...
                releaseGrant?.();
                await delay(300);
                // The stale shadow either never attached or was retired
                // before publication: no shm `connected` event exists and
                // every shadow permit is back.
    ...
    test(
        "owner close cancels shadow publication before closing primary and predecessor",
        async () => {
            const provider = createFakePairedProvider();
            provider.host.onFrame = candidateAutoResponder(RECOVERY_GRANT_TOKEN);
            let releaseGrant: (() => void) | undefined;
    ...
    test(
        "a post-grant failure during a shadow attempt stops the episode permanently",
        async () => {
    ...
    test(
        "shadow permits are bounded at three and returned on failure",
        async () => {
            const provider = recoveryProvider();
