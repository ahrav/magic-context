...
/**
 * Legacy memory-row relocation compatibility. Direct-format production
 * callers use claim relocation above; these helpers remain until U8 removes
 * the migration-era projection.
 *
...
function relocationEnvelope(operation: string, request: unknown): MemoryClaimOperationEnvelope {
    // SAFETY: supported Bun and Node runtimes expose Web Crypto randomUUID.
    const runtimeCrypto = globalThis as unknown as { crypto: { randomUUID(): string } };
    return {
        producer: RELOCATION_PRODUCER,
        operationKey: `${operation}:${runtimeCrypto.crypto.randomUUID()}`,
