...
/**
 * SQL fragment applying the surface decision inside a bounded candidate
 * query, so ineligible rows never consume LIMIT slots (R21). The alias must
 * reference a `memories`-shaped row source.
 */
export function memoryPolicySqlFilter(surface: MemoryPolicySurface, alias: string): string {
    const link = `SELECT 1 FROM legacy_memory_claims lmc
        JOIN claims ON claims.id = lmc.claim_id
        JOIN claim_effective_policy policy ON policy.revision_id = claims.current_revision_id
        WHERE lmc.memory_id = ${alias}.id`;
    if (surface === "explicit_search") {
        // Hard-hidden and review-only rows are excluded; unlinked rows stay
        // (labeled unknown on the compatibility path).
        return `NOT EXISTS (${link} AND (policy.hard_hidden = 1 OR policy.explicit_eligible = 0))`;
    }
    return `EXISTS (${link} AND policy.auto_eligible = 1 AND policy.hard_hidden = 0
        AND policy.policy_version <= ${CLAIM_POLICY_VERSION})`;
}

/** Current claim-policy generation for one project identity, or 0. */
export function readProjectPolicyGeneration(db: Database, projectPath: string): number {
    if (!hasClaimEffectivePolicy(db)) return 0;
    const row = db
        .prepare(
            `SELECT generations.generation AS generation
             FROM claim_project_generations generations
             WHERE generations.project_id = (
                 SELECT id FROM projects WHERE canonical_identity = ?
                 UNION
                 SELECT project_id FROM project_aliases WHERE alias_identity = ?
                 LIMIT 1
             )`,
        )
        .get(projectPath, projectPath) as { generation: number } | null | undefined;
