...
test("rejection receipts match independent operation and digest goldens", () => {
    ...
    try {
        const pairs = [
            ["batch-1", "<invalid>", "4e5b8ce4e465b5263778571420783a6014b06e8ff4210036940f364b96170370"],
            ["batch-2", "<invalid-two>", "a1413710e3a0193519cc53cf25168c7de9198f51b3536b987547e43fc12b7273"],
        ] as const;
        for (const [batchId, rawManifest, requestDigest] of pairs) {
            recordAutonomousManifestRejection({ db, identity: { ...identity, batchId }, rawManifest, reason: "invalid manifest", nowMs: 1 });
            ...
            ).get(`verify:session-1:verify:project:3:${batchId}`) as { operationKey: string; requestDigest: string };
...
test("rejection digest rejects incomplete identity", () => {
    const db = createDirectTestDatabase().db;
    try {
        expect(() => recordAutonomousManifestRejection({
            db,
