// scripts/build-historian-prompt.ts generates this file from historian-prompt.source.md.
// Developers must run `bun run packages/plugin/scripts/build-historian-prompt.ts` to regenerate this file.
//
// Developers must edit `historian-prompt.source.md`, not this file.
...
- \`reason_for_change\` — WHY the old strategy was wrong, in your own words. Not a quote, and not proof that the pivot happened: state the property that made the prior approach unsafe or unworkable ("Redis would add a network dependency this deployment cannot assume"), not the change itself ("the implementation now uses SQLite").
