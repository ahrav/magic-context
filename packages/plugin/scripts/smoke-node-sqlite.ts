// This script smoke-tests the node:sqlite branch of shared/sqlite.ts.
// The transaction() shim supports top-level transactions and nested savepoint rollback.
// Invoke this script with `node packages/plugin/scripts/smoke-node-sqlite.ts`.
import { mkdtempSync, rmSync } from "node:fs";
import { tmpdir } from "node:os";
import { join } from "node:path";
// Node runs this script directly, so imports require explicit .ts extensions.
import {
    appendClaimRevision,
    createClaim,
    createEpisode,
...
try {
    ...
    // bare node:sqlite throws `Unknown named parameter '0'` on `.run([a,b])`.
    // The shim must preserve bun:sqlite positional array binding.
    ...
    try {
        ...
        // The cleanup removes prior rows so later row-count assertions remain valid.
    ...
    // RAISE(ROLLBACK) ends SQLite's transaction; preserve the trigger error rather than issue a second ROLLBACK.
    db.exec(`
    ...
    // The outer transaction commits while the nested savepoint rolls back.
    db.transaction(() => {
        ...
        try {
            ...
            /* The outer transaction continues after catching the inner transaction error. */
        }
    })();
    ...
    // ATTACH works under defensive mode.
    ...
    // Under node:sqlite, the direct claim kernel creates, replays, and revises claims while fencing stale mutations against the registered direct format.
