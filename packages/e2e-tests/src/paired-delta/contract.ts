...
/** Most reason codes name their own terminal path, so pairing one with a different health describes a run that cannot have happened and misattributes the missingness. `harness-failure` and `absence-precondition-unmet` are general-purpose and stay admissible on any non-completed health. */
...
/** Canonical form before folding: composed and decomposed spellings render identically, so a decomposed answer in a turn is a leak of the composed gold. */
const canonicalFold = (value: string): string => value.normalize("NFC").toLowerCase();

/** The production claim encoder rejects a lone surrogate, so a declaration carrying one is unseedable. Uses the runtime predicate where present rather than restating the surrogate ranges. */
...
export interface VerifierContext {
    ...
    /** The `publicClaimId` each declared `locatorIds` handle resolved to for this run. A wire assertion must compare against these, not the symbolic handles: the search turn carries resolved ids, so the handles never appear in the result text. */
...
function parseStringArray(
    ...
    /** Every call site names a set the lane must act on, and every downstream predicate over an empty list is vacuously true. */
...
export function parseScenarioDeclaration(raw: unknown): ScenarioDeclaration {
    ...
    /** Verifiers compare against the model's trimmed file contents, and `p.string` preserves the authored bytes so a fingerprint stays over what was written. A padded answer is therefore unmatchable in every arm; reject it here rather than trimming and changing the covered bytes. */
    if (expectedAnswer !== expectedAnswer.trim()) {
        p.fail("scenario.expectedAnswer: not-trimmed");
    }
    /** Every resolved locator begins with this prefix, so an answer occurring inside it leaks from every id a reseed could produce — the runner's pre-search gate would never clear and the scenario could never execute. Only the prefix is invariant; a collision with the random hex is cleared by reseeding. */
    if (revealsAnswer(expectedAnswer, PUBLIC_CLAIM_ID_PREFIX)) {
        p.fail("scenario.expectedAnswer: collides-with-claim-id-prefix");
    }
    /** The R1 search turn adds this wrapper text to the transcript whatever the query resolves to, so an answer occurring in it is readable by the later probe without any retrieval — and unlike an id collision, no reseed can clear it. */
    /** Composed, not each part alone: the turn emits `${SCRIPTED_SEARCH_PROMPT_PREFIX}${query}` and every query starts with the claim-id prefix, so an answer straddling that join is revealed by a prompt neither string reveals by itself. */
    if ([
    ...
    const suppliesAnswer = (text: string): boolean => {
        ...
        /** A `.` bounded by value characters continues the value rather than ending it, so `47.5` does not supply `47`, while a terminal `.` still does. */
    ...
    /** `scriptedCtxSearchTurn` only performs the fixed oracle exchange; a later user turn has to consume the retrieved result and write the answer file, so without one R1 fails even when retrieval succeeded and its cell is not comparable with the other arms. */
    ...
    /** From the insertion turn inclusive: the ballast precedes that turn, so an answer repeated there is not buried either and stays visible to every arm. Runs after the ordering and evidence-gold rules so a misplaced evidence turn reports its own diagnostic instead of surfacing here. */
    ...
    /** A runner must pass the resolved ids as one locator-shaped query, and `scriptedCtxSearchTurn` rejects a larger set rather than chunking it — chunking would no longer be one tool turn. Enforcing the helper's bound here fails the declaration at freeze time instead of at R1 run time. */
    if (locatorIds.length > ID_SHAPED_QUERY_MAX_TOKENS) {
        p.fail("scenario.interventions.r1.locatorIds: exceeds-search-limit");
    }
    /** The resolved handles become the search query, which `scriptedCtxSearchTurn` interpolates into the model-visible prompt, so a handle named after the answer would let R1 pass its critical check with no retrieval. */
    ...
    for (const [index, { claim }] of memories.entries()) {
        ...
        /** `seedGoldMemories` writes through the production claim encoder, which rejects a lone surrogate, so an ill-formed claim freezes but can never be seeded. */
    ...
    /** `r2.memories` is the declaration's only gold, so it is also what a runner seeds and resolves `r1.locatorIds` against. Unequal lengths leave a handle with no `publicClaimId` to map to and make R1 and R2 compare different gold sets; pairing is positional. */
    ...
    /** Ballast can only bury evidence that already entered the transcript. Evidence supplied at or after the R1 insertion point stays directly available to the probe in every arm, so the baseline answers without retrieval and the absence comparison the lane exists to measure collapses. */
    ...
    /** The mc-on arm forms and later retrieves memory from this turn alone, so an evidence turn that does not carry the answer leaves it nothing to preserve — its failures would measure a missing premise rather than preservation or retrieval. */
    ...
    /** `minimumBallastBytes` is byte-denominated while `modelContextLimit` is token-denominated; ballast below the window rendered at CHARS_PER_TOKEN cannot evict the evidence turn, leaving the absence precondition unsatisfiable. */
...
export function parseArmedCellResult(raw: unknown): ArmedCellResult {
    ...
    /** `criticalCheckIds` is a declared subset of the scenario's checks, so a cell reporting more critical checks — or more critical successes — than overall ones is unreachable and would let an estimator credit critical success against a smaller or zero overall denominator. */
    ...
    /** Every compared arm declares at least one check and at least one critical check, so a completed run cannot report an empty denominator; accepting one would make the paired deltas divide by nothing while looking like a valid measurement. */
    if (
...
export function parsePairedDeltaManifest(raw: unknown): PairedDeltaManifest {
    ...
    const scenarios = p.array(root.scenarios, "manifest.scenarios").map((rawEntry, index) => {
        ...
        /** The manifest is the sole source of run-mode membership, so an entry claiming no mode is frozen into the pool yet selected by no calibration, weekly, or release run — silently removing a scenario from execution while every freeze check still passes. */
    ...
    /** An empty pool matches an empty registry, so a removed or mis-generated index would freeze as valid and leave every run mode with no measurements instead of a contract failure. */
    if (scenarios.length === 0) p.fail("manifest.scenarios: empty");
    /** The registry takes membership solely from here and `assertFrozenPool` permits run-mode edits without drift, so a mode losing its last member would run with no measurements while every freeze check stayed green. This constrains the pool to cover each mode once, not any entry to carry a particular mode. */
...
/** The R3 arm receives its gold in the prompt rather than as memory, so its content is exactly the R2 claim set — deriving it keeps `R3 - R2` a comparison of representation instead of also comparing what each arm was told. */
...
export function validateCheckVector(
    ...
    /** Verifier output is untrusted input, not a parsed artifact: without this revalidation a verifier returning null or a malformed entry raises a bare TypeError that a runner records as a harness failure rather than a scenario failure. */
