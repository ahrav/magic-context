/** The workflow keys its calibration cache on a `hashFiles` glob of this same set, and `calibration-scope.test.ts` holds the two lists equal: a path added here without the workflow would otherwise restore a cached record the runner rejects. */
...
/** The workflow that keys its calibration cache on this scope, relative to the repository root. */
