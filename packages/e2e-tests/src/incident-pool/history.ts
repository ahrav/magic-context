...
export function replayAdjudicationLedger(events: AdjudicationEvent[]): LedgerState {
    ...
    for (const [index, event] of events.entries()) {
        ...
        if (event.seq !== expectedSeq) {
            throw new Error(`${label}: sequence gap for ${event.identity}; expected seq ${expectedSeq}, got ${event.seq}`);
        }
        if (event.supersedes !== null) {
            const target = byEventId.get(event.supersedes);
            if (!target) throw new Error(`${label}: supersedes unknown or later event ${event.supersedes}`);
...
function parseRedactionLines(lines: string[]): EmergencyRedactionEvent[] {
    ...
    return lines.map((line, index) => {
        ...
        if (ids.has(event.event_id)) throw new Error(`${label}: duplicate redaction event id ${event.event_id}`);
...
export function validateIncidentHistory(input: IncidentHistoryInput): IncidentHistoryState {
    const inventory = parseJsonArtifact(input.inventoryText, "source-inventory.json", parseSourceInventory);
    ...
    for (const family of catalog.families) {
        ...
        for (const variant of family.variants) {
            for (const claimId of variant.source_claims) {
                if (!claimIds.has(claimId)) {
                    throw new Error(`orphan variant ${variant.id}: unknown source claim ${claimId}`);
                }
            }
        }
    }
    ...
    for (const event of events) {
        if (!knownIdentities.has(event.identity)) {
            throw new Error(`adjudication ${event.event_id} targets unknown identity ${event.identity}`);
        }
        if (event.kind === "baseline" && !variantById.has(event.identity)) {
            throw new Error(`adjudication ${event.event_id} binds a baseline to non-variant identity ${event.identity}`);
        }
    }
    ...
    for (const variant of variantById.values()) {
        ...
        if (EXECUTABLE_LANES.includes(variant.lane)) {
            ...
            throw new Error(`adjudication-only variant ${variant.id} must not have a baseline adjudication`);
        }
    }
...
function itemRow(item: SourceItem): Record<string, unknown> {
    return { id: item.id, source_path: item.source_path, content_digest: item.content_digest };
}

/** Scalar row of a family (variants are compared individually). */
function familyRow(family: IncidentFamily): Record<string, unknown> {
    return { id: family.id, title: family.title, source_claims: family.source_claims };
}

interface ComparisonContext {
    authorizes: (scope: RedactionScope, targetId: string, oldRow: unknown, newRow: unknown) => boolean;
...
function requireOrderedRow<T extends { id: string }>(
    ...
    const candidateIndex = new Map(candidateRows.map((row, index) => [row.id, index] as const));
    let lastIndex = -1;
    for (const accepted of acceptedRows) {
        const index = candidateIndex.get(accepted.id);
        if (index === undefined) throw new Error(`accepted ${kind} deleted: ${accepted.id}`);
        if (index < lastIndex) throw new Error(`accepted ${kind} reordered: ${accepted.id}`);
        lastIndex = index;
...
function requireRowIntegrity(
    ...
    throw new Error(`accepted ${kind} edited without an appended adjudication or emergency redaction: ${id}`);
}
...
export function compareWithAcceptedSnapshot(
    ...
    for (const [index, line] of accepted.adjudicationLines.entries()) {
        ...
        if (
            ...
            throw new Error(`adjudication ledger prefix rewrote logical identity at line ${index + 1}`);
        }
    ...
    requireOrderedRow(acceptedState.inventory.items, candidateState.inventory.items, "source item", (before, after) => {
        requireRowIntegrity(
            context,
            "source_item",
            before.id,
        ...
        requireOrderedRow(before.claims, after.claims, `source claim of ${before.id}`, (claimBefore, claimAfter) => {
            requireRowIntegrity(
                context,
                "source_claim",
                claimBefore.id,
    ...
    requireOrderedRow(acceptedState.catalog.families, candidateState.catalog.families, "family", (before, after) => {
        requireRowIntegrity(
            context,
            "family",
            before.id,
        ...
        requireOrderedRow(before.variants, after.variants, `variant of ${before.id}`, (variantBefore, variantAfter) => {
            requireRowIntegrity(
                context,
                "variant",
                variantBefore.id,
