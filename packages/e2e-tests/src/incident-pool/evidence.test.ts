...
import { splitLedgerLines, validateIncidentHistory, compareWithAcceptedSnapshot } from "./history";
import {
...
function committedCatalog(): IncidentCatalog {
    return parseIncidentCatalog(JSON.parse(readFileSync(join(INCIDENTS_DIR, "catalog.json"), "utf8")));
}
...
describe("source inventory completeness (R1)", () => {
    it("covers every scanned source item and distinct claim exactly once", () => {
        ...
        expect(byId.get("src-audit-known-issues")!.claims.length).toBeGreaterThanOrEqual(60);
        expect(byId.get("src-parity-findings-s2")!.claims).toHaveLength(16);
        expect(byId.get("src-thinking-block-adjudication")!.claims).toHaveLength(1);
        expect(byId.get("src-pi-todo-declared-red-suite")!.claims).toHaveLength(1);
        expect(byId.get("src-bead-magic-context-x4l-9")!.claims).toHaveLength(2);
    });

    it("records AUDITOR.md as one guide-level row with no finding claims", () => {
        const inventory = committedInventory();
        const guideRows = inventory.items.filter((item) => item.source_path === "AUDITOR.md");
    ...
    it("fails when a single claim is removed from the inventory", () => {
        const inventory = committedInventory();
        const audit = inventory.items.find((item) => item.id === "src-audit-known-issues")!;
        audit.claims = audit.claims.filter((claim) => claim.id !== "claim-audit-a44");
        expect(() => verifySourceCompleteness(inventory, scanSources())).toThrow(
            /source claim missing from inventory: claim-audit-a44/,
        );
    });
    ...
    it("fails when the inventory carries a claim with no live source counterpart", () => {
        ...
        expect(() => verifySourceCompleteness(inventory, scanSources())).toThrow(
            /claim-audit-a99 has no live source counterpart/,
        );
    });

    it("rejects a free-form disposition outside the closed R2 vocabulary", () => {
        const raw = JSON.parse(readFileSync(join(INCIDENTS_DIR, "source-inventory.json"), "utf8")) as {
    ...
    it("rejects an edited accepted inventory row against the repository baseline", () => {
        const accepted = {
            ...
            adjudicationLines: splitLedgerLines(readFileSync(join(INCIDENTS_DIR, "adjudications.jsonl"), "utf8")),
...
describe("mutation evidence normalization (R11)", () => {
    it("derives the accepted 13-artifact/21-record snapshot from live files", () => {
        ...
        expect(view.records.some((record) => record.shape === "mutations")).toBe(true);
        expect(view.records.some((record) => record.shape === "mutation_records")).toBe(true);
    });
    ...
    it("rejects duplicated, malformed, unknown-shape, and orphan-verifier records", () => {
        ...
        try {
            ...
            writeFileSync(join(temp, "mutations", "fm-oc-1.json"), JSON.stringify(duplicated));
            expect(() => loadMutationEvidence(temp, REPO_ROOT)).toThrow(
                /duplicate normalized evidence id ev-fm-oc-1-rung-swap/,
            );

            cpSync(resolve(E2E_ROOT, "mutations", "fm-oc-1.json"), join(temp, "mutations", "fm-oc-1.json"));
            writeFileSync(join(temp, "mutations", "zz-unknown.json"), JSON.stringify({ drills: [] }));
            expect(() => loadMutationEvidence(temp, REPO_ROOT)).toThrow(/unknown mutation artifact shape/);

            writeFileSync(
                join(temp, "mutations", "zz-unknown.json"),
                JSON.stringify({ command: "bun test tests/rust-fm-oc-1.test.ts", mutations: [{ nameless: true }] }),
            );
            expect(() => loadMutationEvidence(temp, REPO_ROOT)).toThrow(/name must be a non-empty string/);
    ...
    it("fails when a mutation link is duplicated into the inventory twice", () => {
        ...
        artifact.claims.push({ ...artifact.claims[0]!, id: "claim-mutation-dg-1-one-byte-input-copy" });
        expect(() => verifySourceCompleteness(inventory, scanSources())).toThrow(
...
describe("ownership matrix (U3 approach 4)", () => {
    ...
    it("fails when an executable claim loses its owner", () => {
        ...
        const family = catalog.families.find((entry) => entry.id === "fam-embedding-freshness")!;
    ...
    it("rejects a binding that names an existing Bun test instead of a scenario module", () => {
        ...
        variant.verifier_binding!.driver = "tests/cache-invariants.test.ts#driveFirstRenderPureDeferStability";
    ...
    it("rejects a live binding whose module or export is missing", () => {
        ...
        expect(() => verifyOwnershipMatrix(committedInventory(), catalog2)).toThrow(
            /does not export driveSomethingElse/,
        );
    });
    ...
    it("rejects giving an unsupported claim an executable target (AE3)", () => {
        const catalog = committedCatalog();
        const variant = findVariant(catalog, "var-a5-archived-reobservation");
        variant.source_claims = [...variant.source_claims, "claim-bead-wrong-dreamer-archival"];
        expect(() => verifyOwnershipMatrix(committedInventory(), catalog)).toThrow(
...
describe("adjudication-only provenance mismatches (AE3)", () => {
    it("keeps wrong-archive and inconsistent-historian-state non-executable and names the missing evidence", () => {
        ...
        for (const [claimId, variantId] of [
            ["claim-bead-wrong-dreamer-archival", "var-wrong-dreamer-archival-claim"],
            ["claim-bead-historian-inconsistent-state", "var-historian-inconsistent-state-claim"],
        ] as const) {
...
describe("synthetic-todo families", () => {
    it("keeps the five Rust variants in one family with distinct normative checks", () => {
        const catalog = committedCatalog();
        const family = catalog.families.find((entry) => entry.id === "fam-synthetic-todo-handoff")!;
        expect(family.variants).toHaveLength(5);
        const checkSets = family.variants.map((variant) => variant.normative_checks.join(","));
        ...
        for (const dependent of family.variants.filter((v) => v.id !== "var-todo-1-synthetic-injection")) {
            expect(dependent.blocked_by).toEqual(["var-todo-1-synthetic-injection"]);
        }
    });

    it("keeps the Pi declared-red suite as a separate family that cannot satisfy the Rust variants", () => {
        const catalog = committedCatalog();
        const rust = catalog.families.find((entry) => entry.id === "fam-synthetic-todo-handoff")!;
        const pi = catalog.families.find((entry) => entry.id === "fam-pi-todo-gap")!;
        expect(pi.id).not.toBe(rust.id);

        const rustClaims = new Set(rust.variants.flatMap((variant) => variant.source_claims));
        const piClaims = new Set(pi.variants.flatMap((variant) => variant.source_claims));
        for (const claim of piClaims) expect(rustClaims.has(claim)).toBe(false);

        const rustChecks = new Set(rust.variants.flatMap((variant) => variant.normative_checks));
        for (const variant of pi.variants) {
            expect(variant.applicability!.harness).toBe("pi");
            for (const check of variant.normative_checks) expect(rustChecks.has(check)).toBe(false);
        }
    });
});
...
describe("verifier-change mutation replay gate (R14)", () => {
    ...
    it("fails contributor verification when a bound mutation no longer produces the expected red result", () => {
        ...
        ).toThrow(/ev-fm-oc-1-rung-deletion did not produce the expected red result/);
        expect(() =>
            assertMutationReplayResults(view, verifier, { "ev-fm-oc-1-rung-swap": true }),
        ...
        for (const family of state.catalog.families) {
            for (const variant of family.variants) {
                ...
                if (variant.lane === "known-red") {
                    expect(baseline?.expected_failed_checks?.length).toBeGreaterThan(0);
                    expect(baseline?.observation_signature).toMatch(/^[0-9a-f]{64}$/);
                }
            }
        }
    });
