...
- `src/features/magic-context/storage-memory-claims-schema.ts`: Migration-owned v84 crosswalk, revision metadata, operation envelope, claim-change outbox, project generation, backfill diagnostics, and transaction-scoped compatibility guard.
...
- `src/features/magic-context/memory/storage-claim-applicability.ts`: Transaction-local applicability writers and readers — idempotent digest-verified stream resolution, head-only assertion appends with non-regressing recorded/knowledge time, memory path-knowledge derivation from `memory_verifications`, and interval-view readers. `memory/source-trust.ts` holds the conservative legacy source-type trust map (`user` → `explicit_user`, everything else → `model_inference`).
...
- `src/features/magic-context/claim-policy-backfill.ts` and `claim-policy-backfill-startup.ts`: Bounded fail-closed v86 seeding — deterministic boundary, resumable cursor batches, conservative VERIFIED/CORROBORATED/CANDIDATE classification, and completion only after count and anti-join reconciliation.
...
- `src/features/magic-context/memory/storage-memory-claims.ts`: Dependency-light transaction-local compatibility kernel used by OpenCode, Pi, dreamer, identity, backfill, and module-mirror adapters; commits claims and `memories` projection effects together.
- `src/features/magic-context/memory/storage-memory-projection.ts`: Leaf legacy projection primitives. Search, mural, dreamer gates, injection, and module serialization continue reading this `memories` surface until U9.
