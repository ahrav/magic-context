...
- Key files: `src/features/magic-context/storage-db.ts`, `src/features/magic-context/fail-closed-block.ts`, `src/features/magic-context/storage-schema-helpers.ts`, `src/features/magic-context/storage-clone.ts`, `src/features/magic-context/storage.ts` (barrel), `src/features/magic-context/migrations.ts`, `src/features/magic-context/message-index.ts`, `src/features/magic-context/search.ts`, `src/features/magic-context/session-project-storage.ts`, `src/features/magic-context/overflow-detection.ts`, `src/features/magic-context/context-authority.ts`, `src/features/magic-context/storage-identity-merge.ts`, `src/features/magic-context/schema-fence-probe.ts`, `src/features/magic-context/dreamer/task-executor.ts`, `src/features/magic-context/dreamer/lease.ts`, `src/features/magic-context/dreamer/manifest-parser.ts`, `src/features/magic-context/dreamer/provider-output-failure.ts`, `src/features/magic-context/memory/project-identity.ts`, `src/features/magic-context/memory/embedding-synapse.ts`, `src/features/magic-context/mural/render-mural.ts`, `src/features/magic-context/user-memory/storage-user-memory.ts`, `src/features/magic-context/smart-notes/wake-plane.ts`, `src/features/builtin-commands/commands.ts`
...
- Contains: The following Rust packages:
  ...
  - `crates/mc-host/`: Direct-linked host library. Owns secure publication, owner-only Unix setup, HMAC authentication, mandatory ring attachment, wire-v2 framing, fixed-profile control and routing, process-global routes and epochs, bounded request settlement split into general and reserved pending/task permit classes, Ping/Pong, and ordered shutdown. `src/composite.rs` dispatches routes across the three static components, `src/synapse/` serves the certified offline embedding lane, and `src/broca/` serves the bounded LLM run lane. Setup or ring failure retires the connection; no application TCP or transport-selection path exists.
...
- `crates/mc-host/src/`: Host ownership boundaries (`instance`, `connection_file`, `setup_socket`, `ring_transport`, `wire`, `control`, `routing`, `dispatch`, `connection`, `runtime`, `composite`, and `harness_closure`) plus public handler and limit contracts; `harness_closure.rs` validates daemon-owned content-addressed OpenCode/Pi graphs, `synapse/` holds the embedding lane, and `broca/` holds the LLM run lane.
- `crates/mc-host/tests/`: Ring client, raw-client, protocol-vector, filesystem-security, routing, dispatch, lifecycle, composite-routing, Synapse, Broca, failure-mode, soak, and real composition coverage, with the committed `fixtures/synapse-tiny/` model bundle.
...
- `src/features/magic-context/memory/project-identity.ts`: Resolve stable project identities (`git:<sha>` or fallback `dir:<md5-12>`) using git root commits or directory hashes, caching directory fallbacks, and utilizing a cooldown period for transient git errors. Supported by `storage-identity-merge.ts` for row-level identity merging with durable audit logging (`identity_merge_log`), `storage-project-identities.ts` for shared identity-table discovery, terminal rekey-chain resolution, and the v82 numeric project registry (`projects`/`project_aliases`), and `packages/plugin/scripts/export-project-identities.ts` for registry seed exports.
- `src/features/magic-context/storage-claims-schema.ts`: Migration-owned v82 DDL for the authoritative claims domain — append-only trigger guards (update/delete/replacement rejection), ownership-safe composite current-pointer FK, and same-project evidence/conflict/verification guards. Dependency-light so the Node SQLite smoke can run it.
- `src/features/magic-context/memory/storage-claims.ts`: Typed transactional writers and fail-closed readers for projects, episodes, source spans, observations, claims, immutable claim revisions, evidence links, conflicts, and verification events. Create/append run under `BEGIN IMMEDIATE` with a current-revision compare-and-swap; a stale writer rolls back with no partial rows. Readers reject direct-SQL null pointers and evidence-less revisions as corruption.
- `src/features/magic-context/storage-current-schema.ts`: Registered direct-format component composer and exact schema inventory.
- `src/features/magic-context/storage-format-epoch.ts`: Direct-format classification, immutable database-incarnation marker, and reset-family state machine.
- `src/features/magic-context/storage-claim-memory-schema.ts`: Claim-owned project-memory attributes, lifecycle, receipts, effects, generations, telemetry, and derivation schema.
- `src/features/magic-context/storage-claim-applicability-schema.ts`: Migration-owned v85 DDL for observation source trust, project-scoped git anchors with typed representations, first-class applicability streams, gapless append-only bitemporal assertions, path/symbol selectors, and the derived `claim_revision_applicability_intervals` view. Dependency-light so the Node SQLite smoke can run it.
- `src/features/magic-context/memory/storage-claim-applicability.ts`: Transaction-local applicability writers and readers with digest-verified streams and gapless assertions.
- `src/features/magic-context/storage-claim-policy-schema.ts`: Migration-owned v86 DDL for revision policy subjects, gapless append-only maturity streams/assertions, disposition events, host-confirmed approval actions, content-addressed enforcement artifacts and revocation events, the rebuildable `claim_effective_policy` projection, projector watermarks, and the derived `claim_maturity_heads` view. Dependency-light so the Node SQLite smoke can run it.
- `src/features/magic-context/memory/claim-policy.ts` and `memory/claim-visibility-policy.ts`: Pure policy modules — the versioned fine-taint classifier, directive-origin restrictions, automatic maturity transition predicates, and the single table-driven effective-visibility evaluator (surfaces, reason codes, sanitized agent labels) shared by every surface integration.
- `src/features/magic-context/memory/storage-claim-policy.ts` and `memory/storage-claim-visibility.ts`: Transaction-local policy write kernel (subjects, maturity appends, dispositions, approvals, artifacts, effective projection, watermarks) and the indexed memory-surface readers (`filterMemoriesByPolicy`, per-surface decisions, labels) consumed by search, `ctx_memory`, injection, and the module mirror.
- `src/features/magic-context/memory/claim-policy-commands.ts`: Host-owned `/ctx-approve` and `/ctx-enforce` workflows shared by OpenCode and Pi — two-step stale-safe confirmation bound to project/revision/digest, idempotent approval and revocation, artifact canonicalization inside the project root, and injectable artifact evaluation. Never exposed as an agent tool.
...
- `src/features/magic-context/migrations.ts`: Historical schema definitions retained as component inputs and unsupported-format fixtures; production open does not migrate old databases.
- `src/features/magic-context/message-index.ts`: FTS-backed raw-message index for `ctx_search`.
- `src/features/magic-context/search.ts`: Exact claim lookup plus retrieval over raw messages, git commits, and session/smart notes.
...
**New Rust handler operation:** add module business logic in `crates/mc-module/src/lib.rs`. Keep socket framing, authentication, route allocation, correlations, settlement, and shutdown ownership in `crates/mc-host/`. Production adapter or binary wiring belongs in `mc-module`; `mc-host` must not depend on `mc-module` (the reverse direction exists only as a dev-dependency for `crates/mc-module/tests/broca_roundtrip.rs`).
