...
- `src/features/magic-context/storage-memory-claims-schema.ts`: Migration-owned v83 crosswalk, revision metadata, operation envelope, claim-change outbox, project generation, backfill diagnostics, and transaction-scoped compatibility guard.
...
- `src/features/magic-context/claims-backfill.ts` and `claims-backfill-startup.ts`: v83 eager/lazy row adoption, relationship translation, anti-join reconciliation, startup resume, v22 takeover, and doctor recovery. Production nonempty eager cutoff remains zero pending reviewed calibration.
- `src/features/magic-context/memory/storage-memory-claims-crash.test.ts` and `memory/fixtures/claims-crash-worker.ts`: Bun/Node two-phase process-crash matrix over live WAL databases; checked evidence is `docs/evidence/claims-backfill/v83-process-crash.json`.
